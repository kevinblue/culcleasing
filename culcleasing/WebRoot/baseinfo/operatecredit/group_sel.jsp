...
String credit_type_id=getStr(request.getParameter("ctype_id"));
...
//out.print("aaaaaaaaaaa="+credit_group_name);
...
{
	sqlstr="select * from base_department where id not in (select id from base_department where charindex(id,'"+seledid+"')>0) order by dept_name asc";
	...
	if (rs.next())
	{
		while (!rs.isAfterLast())
		{
        		if (!seledtemp.equals(getDBStr(rs.getString("id"))))
        		{
        			...
        			selmc=selmc+getDBStr(rs.getString("dept_name"))+",";
        		}
        		rs.next();
    		}
