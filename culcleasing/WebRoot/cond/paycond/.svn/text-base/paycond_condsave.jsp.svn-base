...
if("add".equals(type)){//添加付款前提
	...
	//2.1先删除付款前提
	sqlstr = "delete from proj_fund_fund_charge_condition_temp where payment_id='"+item_id+"'";
	db.executeUpdate(sqlstr);
	
	System.out.println("ckAmount"+ckAmount);
	...
	if("0".equals(ckAmount)){
		...
		for(int i=0;i<cond_id.length;i++){
			sqlstr += " insert into proj_fund_fund_charge_condition_temp(payment_id,pay_condition,status,remark)";
			sqlstr += " select '"+item_id+"','"+cond_id[i]+"','未收',''";
		}
