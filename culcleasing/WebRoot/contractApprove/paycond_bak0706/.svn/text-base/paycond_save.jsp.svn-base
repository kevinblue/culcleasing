...
double alCalMoney = 0;
int fee_num = 0;
sqlstr = "select sum(isnull(plan_money,0)) as alCalMoney,count(fee_num) as fee_num from proj_fund_fund_charge_plan_temp";
sqlstr+= " where proj_Id='"+proj_id+"' and measure_id='"+doc_id+"' and fee_type='"+fee_type+"'";
rs = db.executeQuery(sqlstr);
if(rs.next()){
...
if("11".equals(fee_type)){//判断首付款是否超出
	...
	fee_type_name = "咨询费支出";
	cond_plan_money = consulting_fee;
...
if(outFlag==0){//没有超出则插入数据库
	...
	// 付款 -- 设备款
	sqlstr+=" select '"+doc_id+"','"+proj_id+"','"+pay_type+"','"+fee_type+"','"+(fee_num+1)+"','"+plan_date+"','"+plan_money+"','"+currency+"','"+pay_obj+"',";
	sqlstr+=" null,'"+pay_bank_no+"','"+pay_way+"','"+fpnote+"','"+dqczy+"','"+datestr+"','"+dqczy+"','"+datestr+"'";
	flag = db.executeUpdate(sqlstr);
	LogWriter.logDebug(request, "插入新的资金计划款项名称："+fee_type_name+" 款项金额："+plan_money+"___"+sqlstr);
	
	String sqlLog = LogWriter.getSqlIntoDB(request, "项目立项", "资金计划", "插入新的资金计划款项名称："+fee_type_name+" 款项金额："+plan_money, sqlstr.substring(0,20));
