...
//2.1先查询出已经生产资金计划的金额
double alCalMoney = 0;
sqlstr = "select sum(isnull(plan_money,0)) as alCalMoney from proj_fund_fund_charge_plan_temp";
sqlstr+= " where proj_Id='"+proj_id+"' and measure_id='"+doc_id+"' and fee_type='"+fee_type+"' and payment_id<>'"+item_id+"' ";
rs = db.executeQuery(sqlstr);
if(rs.next()){
...
if(outFlag==0){//没有超出则插入数据库
	...
	sqlstr = "update proj_fund_fund_charge_plan_temp set pay_obj='"+pay_obj+"',plan_date='"+plan_date+"',plan_money='"+plan_money+"',currency='"+currency+"',pay_bank_no='"+pay_bank_no+"',";
	sqlstr +="pay_type='"+pay_type+"',fpnote='"+fpnote+"',modificator='"+dqczy+"',modify_date='"+datestr+"' where payment_id='"+item_id+"'";
