...
<%
	...
	//2012-3-29
	String ratio_param=getStr(request.getParameter("ratio_param"));
	//1.1.5基础参数
	...
	ConditionMediBean conditionMediBean = new ConditionMediBean(
	 doc_id,  "",  equip_amt,currency,  lease_money,  first_payment,caution_money,  "",
	...
	//2012-03-29
	conditionMediBean.setRatio_param(ratio_param);
	System.out.println("ddddddddddddddddddddddddd"+conditionMediBean.getRatio_param());
	
	LogWriter.logDebug(request, conditionMediBean.getContract_id()+"保存之前操作，封转Bean。。。。"+saveType);
	
	//================2.保存ConditionBean?insert:update================
	int flag = 0;	
	flag = ConditionMediService.saveConditionContractBeanIntoTemp(conditionMediBean, saveType);
	if( "是".equals(is_floor) ){//判断是否保底
		RentInfoBox rentInfoBox = RentPlanService.calcMediContRentPlan(conditionMediBean);
		
		//================4.保存保底 租金计划、现金流================
		flag += RentInfoDBOperation.operMediContRentInfoBox2DB(rentInfoBox, conditionMediBean);//保存
			
		LogWriter.logDebug(request, "起租租金测算完成，保存租金计划、现金流成功！");
		
	}else{ //非保底无需要操作
		System.out.println("非保底租金项目");
	}
