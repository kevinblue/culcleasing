...
<form action="qrzxlx_list.jsp" name="dataNav" onSubmit="return goPage()">
...
String sqlstr = "select * from (select info.contract_id,dbo.getcustnamebycontractid(info.contract_id) as cust_name,cond.lease_money,(select sum(interest) from fund_rent_plan where contract_id=info.contract_id) as all_inerest,cond.year_rate,(select straight_interest from vi_fin_fund_rent_plan where contract_id=info.contract_id and fin_plan_date>='"+star+"' and fin_plan_date<'"+end+"') as straight_interest from contract_info as info left join contract_condition as cond on (info.contract_id=cond.contract_id)) as rs where rs.all_inerest>0"; 
