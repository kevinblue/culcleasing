...
<title>当月新增合同 - 财务</title>
<link href="../../css/global.css" rel="stylesheet" type="text/css">
<script src="../../js/comm.js"></script>
</head>

<body >
<form action="xzht_list.jsp" name="dataNav" onSubmit="return goPage()">
...
<%
ResultSet rs;
String sqlstr = "select * from (select info.contract_id,dbo.getcustnamebycontractid(info.contract_id) as cust_name,cond.lease_money,(select sum(interest) from fund_rent_plan where contract_id=info.contract_id) as all_inerest,cond.year_rate from contract_info as info left join contract_condition as cond on (info.contract_id=cond.contract_id) left join contract_finstartdate as fin on(info.contract_id=fin.contract_id) where fin.finstart_date>=dateadd(dd,-datepart(dd,"+getSystemDate(1)+")+1,getdate())) as rs where rs.all_inerest>0"; 
...
%>  
      <tr class="cwDLRow" >
		...
		<td align="center"><%=formatNumberDoubleTwo(rs.getDouble("year_rate")) %>%</td>
      </tr>
<%
		rs.next();
		i++;
