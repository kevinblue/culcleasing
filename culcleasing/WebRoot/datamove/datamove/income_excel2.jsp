...
db.executeUpdate(sqlstr);
//删除资金表中保证金抵扣租金的数据
sqlstr="delete from fund_fund_charge where contract_id='"+p_contract_id+"' and fee_type='108' and item_method ='退款'";
db.executeUpdate(sqlstr);
...
for(int i=0;i<list.size();i++){
	...
	sqlstr="select rent,fin_date,hire_date,balance_mode,instead_flag,ebank_number from etemp_fund_rent where contract_id='"+contract_id+"' order by hire_date";
	rs=db.executeQuery(sqlstr);
	while(rs.next()){
		...
		ebank_number=getDBStr(rs.getString("ebank_number"));

		if(balance_mode.equals("是")){
			balance_mode="BalanceWay8";
		}else{
			balance_mode="";
		}
		
		if(instead_flag.equals("是")){
			instead_flag="是";
		}else{
			instead_flag="否";
		}
		...
		while(rs1.next()){
			...
			if(rnddouble(Double.parseDouble(rent),2)<=rnddouble(Double.parseDouble(interest_overage),2)){
				...
				sqlstr="insert into fund_rent_income(contract_id,plan_list,hire_list,hire_date,accounting_date,rent,corpus,interest,balance_mode,instead_flag,ebank_number) select '"+contract_id+"',"+rent_list+","+hire_list+",'"+hire_date+"','"+fin_date+"',"+rent+",0,"+rent+",'"+balance_mode+"','"+instead_flag+"','"+ebank_number+"'";
				...
				sqlstr="insert into fund_rent_income(contract_id,plan_list,hire_list,hire_date,accounting_date,rent,corpus,interest,balance_mode,instead_flag,ebank_number) select '"+contract_id+"',"+rent_list+","+hire_list+",'"+hire_date+"','"+fin_date+"',"+rent+","+String.valueOf(rnddouble(Double.parseDouble(rent)-Double.parseDouble(interest_overage),2))+","+interest_overage+",'"+balance_mode+"','"+instead_flag+"','"+ebank_number+"'";
				db2.executeUpdate(sqlstr);
