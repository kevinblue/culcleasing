...
function loadCateXML(){
   ...
   var vXSLName=WpsRootPath+"/xsl/view_LDAPlist.xsl";  	
   var v_src=WpsRootPath+"/servlet/tree?type=p&isuse=true"+cgi;
   source.load(v_src);   
   style.load(vXSLName);
...
function getResearchXML(){
   ...
   if (TreeStartNode && TreeStartNode!="" && TreeStartNode!="root")
          vStart="&id="+TreeStartNode;
   else
          vStart=""; 
   
   var v_src=WpsRootPath+"/servlet/tree?type=p"+vStart+"&key="+vKey;   
   source.load(v_src);
...
function setPageCount(){
  try{	
  var pageCount=document.all.pageCount.value;
  var vTag = document.all.FrameSetNumber;	
  for(var i=0;i<vTag.options.length;i++){
    if(pageCount==vTag.options[i].value){
     	vTag.options[i].selected=true;break;
    } 	
  }	 
  }catch(E){}
}	