...
ResultSet rs1;
ResultSet rs2;
String wherestr = " where 1=1 and dbo.bb_getRemEbankMoney(fund_ebank_data.ebdata_id)>0 and fund_ebank_data.status='有效' and fund_ebank_data.business_flag='是'";
...
sqlstr = "select isnull(dbo.bb_getRemEbankMoney(fund_ebank_data.ebdata_id),0) as remEbankMoney,fund_ebank_data.ebdata_id, fund_ebank_data.contract_id, fund_ebank_data.order_number, fund_ebank_data.arrive_date, fund_ebank_data.account_bank, fund_ebank_data.acc_number, fund_ebank_data.client_name, fund_ebank_data.client_accnumber, isnull(fund_ebank_data.fact_money,0) as fact_money_show, isnull(dbo.bb_getRemEbankMoney(fund_ebank_data.ebdata_id),0) as fact_money, fund_ebank_data.summary, fund_ebank_data.sn, fund_ebank_data.status, fund_ebank_data.business_flag from fund_ebank_data" + wherestr; 
...
<table border="0" cellspacing="0" cellpadding="0" >    
    <tr class="maintab">
		...
		金额：从<input name="fact_money_start" type="text" size="10" value="<%= fact_money_start%>">到<input name="fact_money_start" type="text" size="10" value="<%= fact_money_start%>">
...
if ( rs.next() ) {
	...
	if(end_flag.equals("gongchao")){	//客户帐号匹配
		if(!client_accnumber.equals("")){
			sqlstr="select contract_id,(select isnull(sum(rent),0) from fund_rent_plan where contract_id=contract_info.contract_id and plan_date<='"+arrive_date+"')-(select isnull(sum(rent),0)+isnull(sum(rent_adjust),0) from fund_rent_income where contract_id=contract_info.contract_id and hire_date<dateadd(day,1,'"+arrive_date+"')) as badRent from contract_info where cust_id=(select cust_id from cust_account where acc_number='"+client_accnumber+"')";
			System.out.println("sqlstr1=================="+sqlstr);
			rs1=db1.executeQuery(sqlstr);
			while(rs1.next()){
				end_flag="1";
				contract_id=getDBStr( rs1.getString("contract_id") );
				badRent=getDBStr( rs1.getString("badRent") );
				if(Double.parseDouble(badRent)>0){
					sqlstr="select (select isnull(sum(rent),0) from fund_rent_plan where contract_id='"+contract_id+"' and plan_date<='"+arrive_date+"')-(select isnull(sum(rent),0)+isnull(sum(rent_adjust),0) from fund_rent_income where contract_id='"+contract_id+"' and hire_date<dateadd(day,1,'"+arrive_date+"'))-"+fact_money+" as adjust_flag";
					System.out.println("sqlstr2=================="+sqlstr);
					rs2=db2.executeQuery(sqlstr);
					if(rs2.next()){
						adjust_flag=getDBStr( rs2.getString("adjust_flag") );
						if(Double.parseDouble(adjust_flag)==0){
	...
	if(end_flag.equals("0")){	//客户名称匹配
		if(!client_name.equals("")){
			sqlstr="select contract_id,(select isnull(sum(rent),0) from fund_rent_plan where contract_id=contract_info.contract_id and plan_date<='"+arrive_date+"')-(select isnull(sum(rent),0)+isnull(sum(rent_adjust),0) from fund_rent_income where contract_id=contract_info.contract_id and hire_date<dateadd(day,1,'"+arrive_date+"')) as badRent from contract_info where cust_id in (select cust_id from vi_cust_all_info where cust_name like '%"+client_name+"%')";
	...
	if(end_flag.equals("gongchao")){	//金额匹配
		sqlstr="select contract_id,(select isnull(sum(rent),0) from fund_rent_plan where contract_id=contract_info.contract_id and plan_date<='"+arrive_date+"')-(select isnull(sum(rent),0)+isnull(sum(rent_adjust),0) from fund_rent_income where contract_id=contract_info.contract_id and hire_date<dateadd(day,1,'"+arrive_date+"')) as badRent from contract_info";
...
%>
      ...
      <tr>
		...
		<td><%= process_info %></td>
		<td><a href="ebankHl.jsp?arrive_date=<%= arrive_date %>&contract_id_show=<%= contract_id_show %>" target="_blank"><%= contract_id_show %></a></td> 
		<%
