...
String bank_name = fileBean.getFieldValue("bank_name");
String bank_no = fileBean.getFieldValue("bank_no");
upload_date = ebank_date;
//到账银行、到账账户
own_bank = bank_name;
own_acc_number = bank_no;
//判断是否一天只能上传一次 --暂时屏蔽
int dateflag=0;
...
sqlstr="select isnull(max(cast(no as int)),1)+1 as no from GENERATE_NO where generate_type='网银数据上传'";
...
eup_id = datestr+"-"+eup_id;
//上传基础信息			
sqlstr = "insert into fund_ebank_imp(up_id,upload_time,file_name,file_path,bank_name,bank_no,creator,create_date,modificator,modify_date)";
sqlstr+= "values('"+eup_id+"','"+ebank_date+"','"+saObjectFile[0]+"','"+filePath+"','"+bank_name+"','"+bank_no+"','"+czy+"','"+datestr+"','"+czy+"','"+datestr+"')";
db.executeUpdate(sqlstr);
		...
		if(saObjectFile[j]!=null&&!saObjectFile[j].equals("")){
			...
			if(execlBean.getFlag()){
				...
				for(int row=0;row<obj.length;row++){//遍历多行
					...
					summary=objrow[1];
					fact_money = objrow[2];
					client_acc_number = objrow[3];
					client_name = objrow[4];
...
//					if(!flag){
					...
					//生成网银编号
					sqlstr="select count(id)+1 as no from generate_no where generate_type='网银数据'";
					rs=db.executeQuery(sqlstr);
					if(rs.next()){
						ebdata_id=getDBStr( rs.getString("no") );
					}rs.close();
					sqlstr="insert into generate_no select '网银数据','"+datestr+"','"+ebdata_id+"'";
					db.executeUpdate(sqlstr);
					ebdata_id = eup_id+"-"+ebdata_id;
...
%>
<%if(null != db1){db1.close();}%><%if(null != db2){db2.close();}%>