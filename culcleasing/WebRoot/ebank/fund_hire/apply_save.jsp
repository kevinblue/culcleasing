...
if( "addEbankFund".equals(stype) ){//添加首付款
	//生成收款单号
	String glide_id="";
	sqlstr="select isnull(max(cast(no as int)),1)+1 as glide_id from GENERATE_NO where generate_type='网银资金收款'";
	rs=db.executeQuery(sqlstr);
	if(rs.next()){
		glide_id=getDBStr( rs.getString("glide_id") );
	}rs.close();
	sqlstr="insert into generate_no select '网银资金收款','"+systemDate+"','"+glide_id+"'";
	...
	sqlstr+="values(";
	sqlstr+="'"+glide_id+"','网银资金收款','未提交','未通过','未核销','"+amt+"','"+amount+"','"+systemDate+"',";
	...
	sqlstr+=" select id,'"+glide_id+"',contract_id,'"+dqczy+"','"+systemDate+"' from contract_fund_fund_charge_plan ";
	sqlstr+=" where id in("+sqlIds+") ";
	flag += db.executeUpdate(sqlstr);
	//修改资金计划表flag字段值为2
	sqlstr = "Update contract_fund_fund_charge_plan set flag=2 where id in("+sqlIds+")";
	flag += db.executeUpdate(sqlstr);
	
	//日志操作
	String sqlLog = LogWriter.getSqlIntoDB(request, "资金投放管理", "网银资金收款", dqczy+"用户进行网银资金收款，收款金额:"+amt+", 款项数量："+amount, sqlstr);
