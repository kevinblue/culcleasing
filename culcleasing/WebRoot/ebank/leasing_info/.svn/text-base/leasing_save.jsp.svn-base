...
if ( stype.equals("add") ){ //核销首付款
	...
	if (sqldata.indexOf("_")>-1) {
		...
		for (int i=0;i<s.length;i++){
			...
			sqlstr +=" where funds_mode='收款' and funds_type not in('管理服务费','DB保证金') and fund_fund_charge_plan.proj_id in ( ";
			sqlstr +=" 	select proj_id from apply_info_detail where apply_id ="+sid_arr[i]+")";
		}
		...
		sqlstr +=" left join vi_item_method on vi_item_method.pl_id=fund_fund_charge_plan.id  ";
		sqlstr +=" where funds_mode='收款' and funds_type not in('管理服务费','DB保证金') and fund_fund_charge_plan.proj_id in ( ";
		sqlstr +=" 	select proj_id from apply_info_detail where apply_id ="+sid+")";
	...
	sid_sql+=" and funds_mode='收款' and funds_type not in('管理服务费','DB保证金') ";
	...
	//租金实收表修改数据
	sqlstr="insert into fund_rent_income(proj_id,plan_list,hire_date,rent,corpus,interest,rent_type,item_method) ";
	sqlstr+=" select proj_id,'1',fact_date,fact_money,fact_money,'0','租金','非网银' from fund_fund_charge where ";
