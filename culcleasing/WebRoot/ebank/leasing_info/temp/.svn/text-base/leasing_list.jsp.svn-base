...
function validata() {
	...
	for(i=0;i<names.length;i++){
		...
		if (fd.length==0) {
			...
			sql+="'"+fid+"','"+fd+"','"+yh+"','"+zh+"_";
			id_s+="'"+fid+"',";
			sql_date+=" update apply_info set status='已核销',fact_date='"+fd+"' where id='"+fid+"' and pay_type in('资金','资金管理服务费')";
		}
	}
	if (flag==0 && isAdd==-1) {
		//拼凑执行的sql语句
...
sqlstr="select apply.*,dl.skyy as yh,dl.skzh as zh,title,khmc from apply_info  apply left join dl_mpxx dl  on apply.dld_id=dl.khbh left join dbo.ifelc_conf_dictionary on ifelc_conf_dictionary.name=apply.pay_method "+wherestr+" and pay_type in('资金','资金管理服务费') and apply.status<>'已驳回'  and isnull(is_sub,'')='已提交'  order by apply.status ";
...
<table border="0" width="100%" id="table8" cellspacing="0" cellpadding="0" style="margin-top:2px;">
	...
	<table border="0" cellspacing="0" cellpadding="0">
		<tr class="maintab">
			<td nowrap>
			<BUTTON class="btn_2" name="btnHire" value="核销"  type="button" onclick="validata();">
			<img src="../../images/sbtn_new.gif" align="absmiddle" border="0">&nbsp核销</button>
			</td>
			<td>								
			...
			<td nowrap>
			<input name="ck_all" type="checkbox">&nbsp全选
...
if ( rs.next() ) {
	...
	String flag= getDBStr(rs.getString("status"));
	
%>
	<tr>				
		<td>
		 <%
        	if ("已核销".equals(flag.trim())){
         %>
	   ...
	   <td align="center"><%=getDBStr(rs.getString("title")) %></td>
        ...
        <%
			if ("已核销".equals(flag.trim())){
        %>
        <%=getDBDateStr(rs.getString("fact_date")) %>
        <%} else { %>
      	<input name="fact_date" type="text" size="15" id="<%=fact_date %>"  Require="true"   readonly maxlength="10" dataType="Date"> <img  onClick="openCalendar(<%=fact_date %>);return false" style="cursor:pointer; " src="../../images/tbtn_overtime.gif" width="20" height="19" border="0" align="absmiddle"></td>
        <%} %>
