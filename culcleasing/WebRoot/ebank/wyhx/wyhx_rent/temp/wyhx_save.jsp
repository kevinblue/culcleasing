			...
			for(int row=0;row<obj.length;row++){//从第五行开始
				...
				if(bank_flag!=null && !"".equals(bank_flag) && "成功".equals(bank_flag)){//回执扣款成功
					...
					if (memo.indexOf("_")>-1) {//罚息时
						sqlstr=" insert into fund_rent_income(proj_id,plan_list,hire_date,rent,corpus,interest,hire_object,hire_account,hire_number,penalty,rent_type,item_method,remark)";
						sqlstr+=" select a.proj_id,rent_list,'"+fact_date+"',"+0+","+0+",penalty,b.account_name,'中国建设银行',b.account,penalty,'违约金','网银',a.hg_remark from fund_rent_plan a left ";
					...
					//修改状态
					sqlstr =" update fund_rent_plan set show_flag=NULL,show_flag2=NULL,export_flag=NULL,modificator='"+czy+"',modify_date='"+datestr+"' where proj_id in (select proj_id from fund_rent_plan where id='"+plan_id+"' )  ";		
					...
					//＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊失败＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊
					//失败次数加1 = 失败原因、余额
					sqlstr= " update fund_rent_plan set show_flag=NULL,show_flag2=NULL,export_flag=NULL,fail_reason='"+fail_reason+"',modificator='"+czy+"',modify_date='"+datestr+"',";
					...
					flag = db.executeUpdate(sqlstr);
					//状态
					sqlstr= " update fund_rent_plan set show_flag=NULL,show_flag2=NULL,export_flag=NULL";
					sqlstr+=" from fund_rent_plan frp where frp.proj_id in (select proj_id from fund_rent_plan where id='"+plan_id+"' )  ";		
					flag = db.executeUpdate(sqlstr);
				}
			}
		}else{
		...
		if( contStr!=null && !"".equals(contStr) ){
			...
			if(bank_flag!=null && !"".equals(bank_flag) && "1".equals(bank_flag)){//回执扣款成功
				...
				if (memo.indexOf("_")>-1) {//罚息时
					...
					sqlstr+=" rent_type,item_method,remark) select a.proj_id,rent_list,'"+fact_date+"',"+0+","+0+",penalty,b.account_name,'中国民生银行',b.account,penalty,'违约金','网银',a.hg_remark from fund_rent_plan a ";
				...
				sqlstr =" update fund_rent_plan set show_flag=NULL,show_flag2=NULL,export_flag=NULL where proj_id in (select proj_id from fund_rent_plan where id='"+plan_id+"' )  ";		
				System.out.println("sqlstr==================="+sqlstr);
				flag = db.executeUpdate(sqlstr);
