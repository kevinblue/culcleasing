	...
	for(int i=0;i<iCount;i++){
		...
		if(execlBean.getFlag()){
			...
			for(int row=0;row<obj.length;row++){//从第五行开始
				...
				//判断plan_id是否为最小的还租期数
				sqlstr = "select min(rent_list) from dbo.vi_t103_rentinfo ";
				sqlstr+= "where hire_status='未核销' and proj_id=(select proj_id from fund_rent_plan where id='"+plan_id+"') ";
				sqlstr+= "group by proj_id having min(rent_list)=(select rent_list from fund_rent_plan where id='"+plan_id+"')";
...
<script>
		window.close();
		opener.alert("抱歉，备注[<%=errMsg.substring(0, errMsg.length()-1) %>]列租金核销顺序有误,其余租金核销完成");
