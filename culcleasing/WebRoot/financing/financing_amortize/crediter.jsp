...
<td class="tree_title_txt"   valign="middle" align="center">选择授信单位</td>
</tr>
...
{
	wherestr=" and crediter like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="crediter.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>授信单位: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr="select distinct crediter from financing_credit where 1=1  "+wherestr+" order by crediter";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('crediter','crediter');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("crediter"))%>" returnlist="<%=getDBStr(rs.getString("crediter"))%>">
	<%=getDBStr(rs.getString("crediter"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('crediter','crediter');">  
