...
if(saObjectFile!=null){
	...
	System.out.println("22222");
				...
				for(int row=0;row<obj.length;row++){//遍历多行
					...
					sqlstr+= " '"+refund_interest+"','"+refund_money+"','"+refund_otherfee_money+"','"+refund_otherfee_type+"','"+refund_subtotal+"','"+refund_remark+"' ";
					
					LogWriter.logDebug(request, "插入还款计划数据："+sqlstr.substring(0,15));
...
<script type="text/javascript">
	window.close();
	opener.alert("还款计划上传失败，请查看格式是否符合！");
	opener.parent.location.reload();
</script>
<%
}%>