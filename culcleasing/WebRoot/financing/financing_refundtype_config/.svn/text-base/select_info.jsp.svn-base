...
<td class="tree_title_txt"   valign="middle" align="center">选择贷款类型</td>
</tr>
...
{
	wherestr=" and loan_name like '%"+filtervalue+"%'";
}
%>
...
<%
String sqlstr="select * from financing_config_loantype where 1=1 "+wherestr+" order by id";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('form1.loan_code','form1.loan_name');" style="height:80%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("loan_name"))%>" returnlist="<%=getDBStr(rs.getString("code"))%>">
	<%=getDBStr(rs.getString("loan_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('form1.loan_code','form1.loan_name');">  
