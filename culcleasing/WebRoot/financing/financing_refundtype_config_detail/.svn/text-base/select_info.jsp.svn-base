...
<td class="tree_title_txt"   valign="middle" align="center">选择还款方式</td>
</tr>
...
{
	wherestr=" and refund_name like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="select_info.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>贷款类型: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr="select loan.loan_name,refund.* from financing_config_refundtype refund LEFT JOIN financing_config_loantype loan "+
			"ON refund.loan_code=loan.code where 1=1 "+wherestr+" order by id";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('form1.refund_code','form1.refund_name');" style="height:80%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("refund_name"))%>" returnlist="<%=getDBStr(rs.getString("code"))%>">
	<%=getDBStr(rs.getString("refund_name"))%>-<%=getDBStr(rs.getString("loan_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('form1.refund_code','form1.refund_name');">  
