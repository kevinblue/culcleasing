...
<BODY>
		<%
			...
			String fxr_date = getStr(request.getParameter("fxr_date"));
		...
		if (savaType.equals("add")) {
			...
			sql2 +="rate_diff="+rate_diff+",interest_day_amount="+interest_day_amount+",interest_diff='"+interest_diff+"',fact_bank_diff='"+fact_bank_diff+"'"
			;
			if(fxr_date != null && !fxr_date.equals("")){
				sql2+=" ,fxr_date = '"+fxr_date+"'";
			}else{
				sql2+=" ,fxr_date = null";
			}
			sql2+=",modifactor='"+dqczy+"',modify_date=getDate() where id='"+id+"'";
			flag += db.executeUpdate(sql2);
