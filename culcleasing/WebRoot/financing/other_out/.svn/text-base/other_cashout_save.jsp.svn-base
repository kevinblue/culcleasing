...
if ( stype.equals("add") ){        //添加操作
	...
	sqlstr += " values('"+start_date+"','"+end_date+"','"+cash_money+"','"+remark+"','其他流出','"+dqczy+"','"+datestr+"','"+dqczy+"','"+datestr+"')";
	//System.out.println(sqlstr);
	flag = db.executeUpdate(sqlstr);
	
	String sqlLog = LogWriter.getSqlIntoDB(request, "其他操作", "其他流出制定", "新增：录入人员："+dqczy, sqlstr);
	...
	String sqlLog = LogWriter.getSqlIntoDB(request, "其他操作", "其他流出制定", "删除，id号为："+id, sqlstr);
	db.executeUpdate(sqlLog);
	db.close();
	...
	String sqlLog = LogWriter.getSqlIntoDB(request, "其他操作", "其他流出制定", "修改，id号为："+id, sqlstr);
	db.executeUpdate(sqlLog);
	db.close();
