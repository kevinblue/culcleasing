...
<%
	...
	//央行基准利率表
	String sqlstr = " select * from fund_standard_interest"; 
%>
...
%>
      <tr>
		...
		<td align="center" nowrap><%= getDBStr(rs.getString("rate_half"))%></td>
		...
		<%
			if("是".equals(getDBStr(rs.getString("fina_adjust_flag")))){
			%>
		<a href="tx_showAll_ytxR.jsp?txId=<%=getDBStr(rs.getString("id"))%>&adjust_method=RentCalcType1&adjust_flag=是" target=_blank>调息纪录</a>
			<%	
			}else{
		%>
		<a href="tx_showMaintx.jsp?czid=<%=getDBStr(rs.getString("id"))%>&adjust_method=RentCalcType1&adjust_flag=<%=getDBStr(rs.getString("fina_adjust_flag"))%>&start_date=<%=getDBDateStr(rs.getString("start_date"))%>" target=_blank>调息</a>
		<% } %>
		</td>
		
        <td align="center" nowrap><%= getDBStr(rs.getString("fina_adjust_flag"))%></td>	
      </tr>
<%
		rs.next();
		i++;
