...
<div style="vertical-align:top;width:100%;overflow:auto;position: relative; left: 0px; top: 0px"  id="mydiv";>
    	<div id="tablesub">
			...
			src="tx_showAll_ytx.jsp?txId=<%=txid%>&adjust_method=<%= adjust_method %>&adjust_flag=<%= adjust_flag %>&start_date=<%=start_date%>">
			</iframe>
		</div>
	    <div>
	    	<!-- 嵌入iframe到页面tx_showAll_wtx.jsp 所有未调息的列表的页面传入主键 -->
			<iframe frameborder="0" name="con" width="100%" height="400" 
				src="tx_showAll_wtx.jsp?txId=<%=txid%>&adjust_method=<%= adjust_method %>&adjust_flag=<%= adjust_flag %>&start_date=<%=start_date%>">
			</iframe>
		</div>
