...
if("save".equals(type)){//添加项目资料
	...
	//2.1先删除付款前提
	sqlstr = "delete from contract_document_temp where contract_id='"+contract_id+"' and doc_id='"+doc_id+"'";
	...
	if("0".equals(ckAmount)){
		...
		for(int i=0;i<mater_id.length;i++){
			sqlstr += " insert into contract_document_temp(contract_id,doc_id,document_id,text_status,electron_status,remark)";
			sqlstr += " select '"+contract_id+"','"+doc_id+"','"+mater_id[i]+"','否','否',''";
		}
	...
	sqlstr = "delete from contract_document_temp where id='"+item_id+"'";
	flag = db.executeUpdate(sqlstr);
