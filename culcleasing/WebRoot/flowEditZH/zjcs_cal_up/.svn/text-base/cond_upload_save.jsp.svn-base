	   ...
	   if(saObjectFile[j]!=null&&!saObjectFile[j].equals("")){
		  ...
		  if(execlBean.isFlag()){
				...
				**/
				...
				//1.2租期(年) - 第5行第5列
				conditionBean.setLease_term( MathExtend.getLeaseTerm(obj[4][4],"12") );
				System.out.println("TTTT Least_Term111::"+ MathExtend.getLeaseTerm(obj[4][4],"12"));
				//1.3咨询服务费收入 - 第5行第7列
				...
				//3.2年还租次数 - 第7行第5列
				conditionBean.setIncome_number(  MathExtend.round(MathExtend.multiply(obj[6][4], obj[4][4]),0) );
				System.out.println("TTTT Least_Term11999991::"+ MathExtend.getLeaseTerm(obj[4][4],"12"));
				...
				System.out.println("TTTT Least_Term222::"+ MathExtend.divide("12", obj[6][4],0 ));
				...
				conditionBean.setEnd_date(obj[11+Integer.parseInt( conditionBean.getIncome_number())][0]);//结束日期
					System.out.println("TTTT Least_Term555552::"+ conditionBean.getIncome_number() );
				...
				**/
				System.out.println("TTTT Least_Term333::"+  ConvertUtil.parseInt(conditionBean.getIncome_number(),12)+12 );
				...
				//执行保存
				//1.保存conditionBean
				ConditionService.saveUploadConditionContractBean(conditionBean);
				//2.保存租金计划List
				RentPlanService.saveUploadRentPlanContractList(rentPlanList, contract_id, doc_id);
				//3.保存现金流List
				RentCashService.saveUploadRentCashContractList(rentCashList, contract_id, doc_id);
				LogWriter.logDebug(request, "执行上传租金测算Excel成功");
				
				//日志操作
				String sqlLog = LogWriter.getSqlIntoDB(request, "合同租金测算", "数据导入", "用户导入租金测算类型【"+uploadFileName+"】测算", sqlstr);
