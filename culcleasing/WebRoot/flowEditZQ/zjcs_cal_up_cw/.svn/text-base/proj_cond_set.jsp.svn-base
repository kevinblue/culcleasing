...
<%
	...
	ConditionService1.flowInitTableData(proj_id, doc_id);//加载税种字段
	
	//判断执行类型 -- 从数据库查询有数据则upd无则add
	String savaType = "upd";
	
	//1.如果upd则假装ConditionBean
	ConditionBean conditionBean = ConditionService.initConditionBean(proj_id, doc_id);
	conditionBean  = ConditionService1.initConditionBean(conditionBean);
	if(conditionBean!=null){
		System.out.println("============="+conditionBean.getSettle_method());
		   ...
		   <tr>
		    ...
		    <td>
		    	<input name="before_interest" id="before_interest" type="text" value="0"  onchange="changeTwoData()"
		    		dataType="Money" size="13" maxlength="20" maxB="20"  Require="true" readonly="readonly" />
		    		是否算本
		    		<input type="text" name="before_interest_type" value="" size="5" readonly="readonly">
		    		<!-- sys 财务不可选
