...
LogWriter.logDebug(request, "读Excel前删除数据-begin_interest_info："+sqlstr);
...
if(saObjectFile!=null){
	...
	//sqlstr+="('"+begin_id+"','"+zJE+"','"+zQC+"','"+flow_date+"','"+datestr+"','"+czy+"')";
	sqlstr+="update begin_interest_info set subsidy_money='"+zJE+"',subsidy_term='"+zQC+"' where begin_id='"+begin_id+"'";
	flag+=db.executeUpdate(sqlstr);
}
