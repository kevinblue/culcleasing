...
if("add".equals(type)){//添加付款前提
	...
	//2.1先删除付款前提
	sqlstr = "delete from contract_fund_fund_charge_condition where payment_id='"+item_id+"'";
	...
	if("0".equals(ckAmount)){
		...
		for(int i=0;i<cond_id.length;i++){
			sqlstr += " insert into contract_fund_fund_charge_condition(payment_id,pay_condition,status,remark)";
