...
<%
	...
	beginInfoBean.setSettle_method(OperationUtil
			.getSettleMethod(uploadFileName));
		...
		for (int j = 0; j < saObjectFile.length; j++) {
			if (saObjectFile[j] != null && !saObjectFile[j].equals("")) {
				...
				if (execlBean.isFlag()) {
					...
					//beginInfoBean.setLease_term( MathExtend.multiply("12", obj[4][4]) );
					//1.3咨询服务费收入 - 第5行第7列
					...
					//3.2年还租次数 - 第7行第5列
					beginInfoBean.setIncome_number(obj[6][4]);
					...
					beginInfoBean.setIncome_day(CommonTool.datePart(
							beginInfoBean.getRent_start_date(), 3));
					beginInfoBean.setStart_date(obj[11][0]);
					//T.2收益率plan_irr - 第8行第8列
					obj[7][7]  = "7.6";
					beginInfoBean.setPlan_irr(obj[7][7]);
					//T.3净融资额 - 第12行第7列
					beginInfoBean.setActual_fund(ConvertUtil
							.parseInverseNumber(obj[11][6]));

					//--其他计算字段---
					//O.2.1 付租方式 = 租期（月）/年还租次数 - 后台计算
					String iny = MathExtend.divide(beginInfoBean
							.getLease_term(), beginInfoBean
							.getIncome_number(), 0);
					beginInfoBean.setIncome_number_year(iny.substring(
							0, 1));
					...
					beginInfoBean.setCreator(czy);//创建日期
					beginInfoBean.setCreate_date(datestr);//创建人
					...
					for (int row = 12; row < ConvertUtil.parseInt(
							beginInfoBean.getLease_term(), 12) + 12; row++) {//遍历多行
						...
						rentPlanBean.setYear_rate(beginInfoBean
								.getYear_rate());
					...
					//执行保存
					//1.保存beginInfoBean - 不保存beginInfoBean
					...
					//日志操作
					String sqlLog = LogWriter.getSqlIntoDB(request,
							"租金变更", "租金数据导入", "用户导入租金测算类型【"
									+ uploadFileName + "】测算", sqlstr);
					flag += db.executeUpdate(sqlLog);
					flag = 12;
				} else {
...
<script type="text/javascript">
	window.open("http://domino.culc.com/eleasing/PMAgent.nsf/CreateRentChangeNotice22?openagent&fileno=1121&contractid=<%=contract_id%>&docid=<%=doc_id%>&beginid=<%=begin_id%>","","status=no,scrollbars=no,location=no,menubar=yes,resizable=yes");
