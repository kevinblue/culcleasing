...
if(type!=null && !type.equals("") && type.equals("yc")){
	//1.先清空原数据
	sqlstr = "Delete from dbo.fund_cash_medi_yc_con_temp where contract_id='"+contract_id+"' and doc_id='"+doc_id+"'";
			...
			if(saObjectFile[j]!=null&&!saObjectFile[j].equals("")){
				...
				System.out.println("22222");
					...
					for(int row=0;row<obj.length;row++){//遍历多行
						...
						//插入数据
						sqlstr = "Insert into dbo.fund_cash_medi_yc_con_temp(doc_id,contract_id,yc_month,yc_hire_date,cash_money)";
						sqlstr+= " Select '"+doc_id+"','"+contract_id+"','"+yc_month+"','"+yc_hire_date+"','"+yc_money+"'";
 ...
 if(type!=null && !type.equals("") && type.equals("bd")){
 	//1.先清空原数据
	sqlstr = "Delete from fund_cash_medi_bd_con_temp where contract_id='"+contract_id+"' and doc_id='"+doc_id+"'";
	db.executeUpdate(sqlstr);
	LogWriter.logDebug(request, "读Excel前删除数据："+sqlstr);
	...
	if(saObjectFile!=null){
		...
		System.out.println("22222");
					...
					for(int row=0;row<obj.length;row++){//遍历多行
						...
						//插入数据
						sqlstr = "Insert into fund_cash_medi_bd_con_temp(doc_id,contract_id,bd_month,bd_hire_date,cash_money)";
						sqlstr+= " Select '"+doc_id+"','"+contract_id+"','"+bd_month+"','"+bd_hire_date+"','"+bd_money+"'";
