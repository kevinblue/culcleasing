...
$(document).ready(function(){
	...
	$("input[name='rate_float_amt']").val(parseFloat("<%=Double.parseDouble(conditionBean.getRate_float_amt()) %>"));
	$("input[name='rate_float_amt']").blur();
	//alert(parseFloat("<%=Double.parseDouble(conditionBean.getRate_float_amt()) %>"));
	//预计IRR plan_irr
	...
	<div id="TD_tab_0">
		...
		<tr>
		 ...
		 <td scope="row" nowrap>租金开票方式</td>
			...
			<td>
			<select style="width:130" name="insure_type" id="insure_type" Require="true" label="投保方式">
			<%
