			...
			if (saObjectFile[j] != null && !saObjectFile[j].equals("")) {
				...
				if (execlBean.isFlag()) {
					...
					for (int row = 12; row < ConvertUtil.parseInt(
						...
						//根据rent_list 查找过去的银行信息
						String rent_list = objrow[1];
						String sqlstr2 = "select frp.plan_status,frp.rent_list, plan_bank_name,plan_bank_no from fund_rent_plan_temp frp left join invoice_rent_detail i on i.begin_id=frp.begin_id and i.rent_list=frp.rent_list ";
						sqlstr2 += " where frp.begin_id='"+begin_id+"' and doc_id='"+doc_id+"' and frp.rent_list='"+rent_list+"' and oth_remark='租金变更前' order by frp.rent_list";
						LogWriter.logDebug(request, "起租 - 租金变更前 -- 租金计划 -- 根据rent_list 查询sql语句==>> "+sqlstr2);
						rs2 = db2.executeQuery(sqlstr2);
						if(rs2.next()) {
							rentPlanBean.setPlan_bank_name(rs2.getString("plan_bank_name"));
							rentPlanBean.setPlan_bank_no(rs2.getString("plan_bank_no"));
							rentPlanBean.setPlan_status(rs2.getString("plan_status"));
						}else{
