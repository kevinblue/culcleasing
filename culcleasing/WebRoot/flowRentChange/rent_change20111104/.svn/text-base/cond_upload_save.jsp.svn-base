	   ...
	   if(saObjectFile[j]!=null&&!saObjectFile[j].equals("")){
		  ...
		  if(execlBean.isFlag()){
				...
				**/
				...
				String iny = MathExtend.divide( beginInfoBean.getLease_term(), beginInfoBean.getIncome_number(), 0 );
				beginInfoBean.setIncome_number_year(iny.substring(0,1));
				...
				**/
				for(int row=12;row<ConvertUtil.parseInt(beginInfoBean.getLease_term(),12)+12;row++){//遍历多行
				...
				flag += RentPlanService.saveUploadRentChangePlanBeginList(rentPlanList, contract_id,begin_id, doc_id);
				//3.保存现金流List
				...
				//日志操作
				String sqlLog = LogWriter.getSqlIntoDB(request, "租金变更", "租金数据导入", "用户导入租金测算类型【"+uploadFileName+"】测算", sqlstr);
