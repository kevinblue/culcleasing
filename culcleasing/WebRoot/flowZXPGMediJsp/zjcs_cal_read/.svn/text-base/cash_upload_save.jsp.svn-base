...
if(type!=null && !type.equals("") && type.equals("yc")){
	//1.先清空原数据
	sqlstr = "Delete from fund_cash_medi_yc where proj_id='"+proj_id+"'";
			...
			if(saObjectFile[j]!=null&&!saObjectFile[j].equals("")){
				...
				System.out.println("22222");
					...
					for(int row=0;row<obj.length;row++){//遍历多行
						...
						//插入数据
						sqlstr = "Insert into fund_cash_medi_yc(proj_id,yc_month,yc_hire_date,cash_money)";
						sqlstr+= " Select '"+proj_id+"','"+yc_month+"','"+yc_hire_date+"','"+yc_money+"'";
 ...
 if(type!=null && !type.equals("") && type.equals("bd")){
 	//1.先清空原数据
	sqlstr = "Delete from fund_cash_medi_bd where proj_id='"+proj_id+"'";
	db.executeUpdate(sqlstr);
	LogWriter.logDebug(request, "读Excel前删除数据："+sqlstr);
	...
	if(saObjectFile!=null){
		...
		System.out.println("22222");
					...
					for(int row=0;row<obj.length;row++){//遍历多行
						...
						//插入数据
						sqlstr = "Insert into fund_cash_medi_bd(proj_id,bd_month,bd_hire_date,cash_money)";
						sqlstr+= " Select '"+proj_id+"','"+bd_month+"','"+bd_hire_date+"','"+bd_money+"'";
