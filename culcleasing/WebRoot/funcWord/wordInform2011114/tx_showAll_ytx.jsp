...
%>
      <tr>
      	...
      	settle_method="<%=getDBStr(rs.getString("settle_method"))%>" adjust_flag="<%= getDBStr(rs.getString("adjust_flag"))%>" begin_id="<%=getDBStr(rs.getString("begin_id"))%>" /></td>
		...
		<td align="center" nowrap><%= getDBStr(rs.getString("rate_float_amt"))%></td>
       ...
       %>
	    ...
	    <td align="center" nowrap>
        	<a target="_blank" class="docLink" href="http://domino.culc.com/eleasing/PMAgent.nsf/CreateCHireFiles?openagent&fileno=9&contractid=<%=getDBStr(rs.getString("contract_id"))%>&rate=<%=getDBStr(rs.getString("after_rate"))%>&startList=<%=getDBStr(rs.getString("rent_list_start"))%>&measure_id=<%=txId %>" >《下载租金变更通知书》</a>
        </td>
       <%} else{
       %>
