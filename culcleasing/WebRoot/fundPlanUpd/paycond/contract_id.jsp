...
<td class="tree_title_txt"   valign="middle" align="center">选择合同编号</td>
</tr>
...
String wherestr=" and phy_contract_id='"+proj_id+"'";

String filtervalue=getStr(request.getParameter("fv"));
if ((filtervalue!=null) && (!filtervalue.equals("")))
{
	wherestr=" and make_contract_id like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="contract_id.jsp">
...
<%
String sqlstr="select make_contract_id from contract_list_info where 1=1 "+wherestr+" order by id";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('form1.make_contract_id1','form1.make_contract_id');" style="height:80%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("make_contract_id"))%>" returnlist="<%=getDBStr(rs.getString("make_contract_id"))%>">
	<%=getDBStr(rs.getString("make_contract_id"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('form1.make_contract_id1','form1.make_contract_id');">  
