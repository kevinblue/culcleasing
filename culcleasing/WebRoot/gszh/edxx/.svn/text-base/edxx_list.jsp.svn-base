...
ResultSet rs1;
String tmpStr="";
sqlstr1 = "SELECT * FROM v_bankedzh where khzh='"+czid+"'";
rs1=db.executeQuery(sqlstr1); 
if (rs1.next()) 
   tmpStr="账户名称:"+rs1.getString("zhmc")+",账号:"+rs1.getString("khzh")+",期初额度:"+rs1.getBigDecimal("sxed",2)+",当前可用额度"+rs1.getBigDecimal("kyed",2)+"。";
else
   tmpStr="";
rs1.close(); 
...
String expsqlstr; 
ResultSet rs;
String wherestr=" WHERE wsfkzh='"+czid+"' and sz is not null";
String kxmc="";
...
if ((rqstrkl1!=null && rqstrkl1.equals("")==false) && (rqstrkl2!=null && rqstrkl2.equals("")==false) && (rqstrqs1!=null && rqstrqs1.equals("")==false) && (rqstrqs2!=null && rqstrqs2.equals("")==false))
{
   ...
   wherestr=wherestr+" and rq>='"+searchKeyqs1+"' and rq<='"+searchKeyqs2+"' and kxmc like '%兑付%' and xmbh in (select xmbh from v_zj_edxx where klrq>='"+searchKeykl1+"' and klrq<='"+searchKeykl2+"' and dfrq >='"+searchKeyqs1+"' and dfrq<='"+searchKeyqs2+"' and klhx='已核销' and dfhx='未核销')"; 
}

expsqlstr="SELECT rq as 日期,xmbh as 项目编号,xmmc as 项目名称,xmhy as 项目行业,xmbmmc as 部门, kxmc as 款项名称,jsfs as 结算工具,sz as 金额  from v_zj_edxx1 "+wherestr;

%>
	<br><span><font style="font-size:0.85em;color:blue"><%=tmpStr%></span></font><br>
        ...
        <form name="searchbar" action="edxx_list.jsp">
 ...
 onclick="SpecialDataWindow('板块','v_xmjbxx_xmhy','xmhy','xmhy','xmhy','stringfld','xmhy','searchbar.searchKeyhy','searchbar.searchKeyhy');"></td>
        <td nowrap>部门
 ...
 onclick="SpecialDataWindow('事业部门','v_yh_zh','bmmc','bmmc','bmmc','stringfld','bmmc','searchbar.searchKeybmdata','searchbar.searchKeybm');"></td>
    	<td nowrap>项目名称
       	<input name="searchKeymc" accesskey="s" type="text" size="20" value="<%=searchKeymc%>"><br>
       结算工具<input name="searchKeygj" accesskey="s" type="text" size="10" value="<%=searchKeygj%>">&nbsp;&nbsp;&nbsp;<input name="image" type="image" src="../../images/tbtn_searh.gif" alt="查询" align="absmiddle"  onclick="searchbar.submit();"></td>
        </form>
      </tr>
</table>
...
<div id="cwDataNav" >
<form name="dataNav" action="edxx_list.jsp">
