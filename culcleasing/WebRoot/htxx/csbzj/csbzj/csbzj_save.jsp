...
if( Description.indexOf("出错") < 0 ) {
 ...
 //caution_money_ratio=fileBean.getFieldValue("caution_money_ratio");
                sqlstr="insert into mproj_company ( manuf_name ,margin_per ,vendor_payment , min_payment,ensure_payment,margin_amount,deposit_amount,margin_time,margin_reason,deposit_export,export_time,export_reason,contract_id, attachment , creator , create_date , modificator , modify_date ) values ('" + manuf_name + "','"+margin_per+"','" + vendor_payment + "','"+min_payment+"','"+ensure_payment+"','"+margin_amount+"','"+deposit_amount+"','"+margin_time+"','"+margin_reason+"','"+deposit_export+"','"+export_time+"','"+export_reason+"','"+contract_id+"','" + fj + "','" + czy + "','" + datestr + "','" + czy + "','" + datestr + "')";
...
<%
 ...
 contract_id= fileBean.getFieldValue("contract_id");
                ...
                sqlstr = "update mproj_company set min_payment='"+min_payment+"',ensure_payment='"+ensure_payment+"',margin_amount='"+margin_amount+"',deposit_amount='"+deposit_amount+"',margin_time='"+margin_time+"',margin_reason='"+margin_reason+"',deposit_export='"+deposit_export+"',export_time='"+export_time+"',export_reason='"+export_reason+"',attachment='"+fj+"',creator='"+czy+"',create_date='"+datestr+"',modificator='" + czy + "',modify_date='" + datestr + "' where id='" + id + "'";
                db.executeUpdate(sqlstr); 
System.out.println("sqlstr*****"+sqlstr);
				if(fj_del==null || fj_del.equals("")) {
...
%>
				<script>
					...
					window.location.href="csbzj_mod.jsp?id=<%=id%>";
				</script>
<%
		}
           }
           if (stype.indexOf("del")>=0) {
                ...
                sqlstr = "delete from mproj_company where id='" + id + "'";
