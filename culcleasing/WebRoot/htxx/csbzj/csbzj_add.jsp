...
String uid = getStr(request.getParameter("uid"));
...
String sqlstr="select margin_per,initial_margin from contract_manuf where 1=1 and id='"+uid+"' ";
rs = db.executeQuery(sqlstr); 
System.out.println("^^^^^^^^^^^^sqlstr="+sqlstr);
if (rs.next()){

		margin_per = getDBStr( rs.getString("margin_per") );
                System.out.println("***********************"+margin_per);
...
String sql="select top 1 mproj_company.deposit_amount-mproj_company.deposit_export+(case when ensure_payment is NULL then contract_manuf.initial_margin else  mproj_company.ensure_payment  end) as ensure_payment1 from mproj_company left join contract_manuf on contract_manuf.id=mproj_company.uid order by mproj_company.bid desc" ;
rs=db.executeQuery(sql);
System.out.println("###########"+sql);
if(rs.next()){
    ensure_payment=getDBStr(rs.getString("ensure_payment1"));
    System.out.println("DDDDDDDDDD"+ensure_payment);
}
//rs1.close();
...
<input type="hidden" name="ensure_payment" value="<%=ensure_payment%>">
<style type="text/css">
...
function makeV()
{
    ...
    var totals=0.00;
	var initial_margin=parseFloat(document.getElementsByName("initial_margin")[0].value);//初始金额
	...
	//alert(totals);
	if((deposit_export == null || deposit_export == "")||(deposit_amount==null||deposit_amount=="")){
	alert("初始金额、汇入金额、汇出金额不能为空,现金额=初始金额+汇入金额-汇出金额!");
		document.all.deposit_export.focus();
		return false;
	}
	else{
	  totals=totals+initial_margin+deposit_amount-deposit_export;
...
<tr>
		   ...
		   <td>保证金比率：</td>
		   <td><%= margin_per %>%
			<input name="margin_per" type="hidden" value="<%=margin_per%>"></td>
