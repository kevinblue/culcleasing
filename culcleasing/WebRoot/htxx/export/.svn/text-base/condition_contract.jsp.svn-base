...
sqlstr="select sum(rent) as plan_totalrent from fund_rent_plan where contract_id='"+contract_id+"'";
rs=db.executeQuery(sqlstr);
if(rs.next()){
	plan_totalrent=formatNumberStr(getDBStr(rs.getString("plan_totalrent")),"#,##0.00");
}rs.close();
String cust_name="";
sqlstr="select cust_name from vi_contract_info where contract_id='"+contract_id+"'";
rs=db.executeQuery(sqlstr);
if(rs.next()){
...
sqlstr="select contract_condition.* from contract_condition where contract_condition.contract_id='"+contract_id+"'";
	...
	double dequip_amt = 0;
		...
		if(alCash!=null){
			for(int i=0;i<alCash.size();i++){
				...
				cell.setCellStyle(style);
				cell.setCellValue(corpus);
				cell=templateRow.createCell((short)7);
				cell.setCellStyle(style);
				cell.setCellValue(interest);
				cell=templateRow.createCell((short)8);
				cell.setCellStyle(style);
				cell.setCellValue(formatNumberDoubleTwo(dlease_money));
				//少剩余本金
				cell=templateRow.createCell((short)9);
				cell.setCellStyle(style);
				cell.setCellValue(formatNumberDoubleTwo(net_flow));
