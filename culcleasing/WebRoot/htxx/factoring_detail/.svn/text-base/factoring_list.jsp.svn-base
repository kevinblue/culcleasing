...
String sqlstr = "select *,dengjiren=dbo.GETUSERNAME(creator),xiugairen=dbo.GETUSERNAME(modificator) from factoring_contract_info  " + wherestr+" order by create_date "; 
    ...
    <tr class="maintab">
      <td align="left" width="1%"><!--操作按钮开始-->
        <table border="0" cellspacing="0" cellpadding="0" >
			<tr class="maintab">
				<%if(right.CheckRight("factoring_add",dqczy)>0){%><td><a href="#" accesskey="n" onClick="dataHander('add','factoring_add.jsp',dataNav.itemselect);"><img   src="../../images/sbtn_new.gif" alt="新增(Alt+N)" align="absmiddle"></a></td><%}%>
        <%if(right.CheckRight("factoring_mod",dqczy)>0){%><td><a href="#" accesskey="m" onClick="dataHander('mod','factoring_mod.jsp?id=',dataNav.itemselect);"><img   src="../../images/sbtn_mod.gif" alt="修改(Alt+M)" align="absmiddle" ></a></td><%}%>
				<%if(right.CheckRight("factoring_mod",dqczy)>0){%><td><a href="#" accesskey="d" onClick="dataHander('del','factoring_del.jsp?id=',dataNav.itemselect);"><img src="../../images/sbtn_del.gif" alt="删除(Alt+D)" align="absmiddle" ></a></td><%}%>
...
%>
      ...
      <tr>
        ...
        <td align="center"><a href="factoring.jsp?id=<%=getDBStr( rs.getString("id") )  %>" target="_blank"><%=getDBStr( rs.getString("id") )  %></a></td>
        <td align="center"><%= getDBStr( rs.getString("factoring_num") ) %></td>
        <td align="center"><%= getDBStr( rs.getString("cust_name") ) %></td>
        <td align="center"><%= getDBStr( rs.getString("factoring_backname") ) %></td>
		<td align="center"><%= getDBStr( rs.getString("factoring_account") ) %></td>
			<td align="center"><%= getDBDateStr( rs.getString("factoring_start_date") ) %></td>

		<td align="center"><%=formatNumberStr( rs.getString("factoring_rent"),"#,##0.00") %></td>
