...
int canedit=0;
if (right.CheckRight("htxx-htxxcx-list",dqczy)>0) canedit=1;
...
%>
      ...
      <tr onmouseover="fn_changeTrColor()" onmouseout="fn_changeTrColor()">
		...
		<td><%= formatNumberStr(getDBStr( rs.getString("equip_amt") ),"#,##0.00") %></td>
      	<td>
      		<a href="htxxcx_cond.jsp?contract_id=<%= getDBStr( rs.getString("contract_id") ) %>" target="_blank">交易结构</a>
      		<a href="htxxcx_rent.jsp?contract_id=<%= getDBStr( rs.getString("contract_id") ) %>" target="_blank">回笼计划</a>
      		<a href="htxxcx_atl.jsp?contract_id=<%= getDBStr( rs.getString("contract_id") ) %>" target="_blank">实际回笼</a>
      	</td>
      </tr>
<%
		rs.next();
