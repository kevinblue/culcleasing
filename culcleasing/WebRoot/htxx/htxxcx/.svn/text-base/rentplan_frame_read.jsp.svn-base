...
<%
	...
	List l_rent_fx = new ArrayList();
	List l_plan_status = new ArrayList();
	sqlstr="select rent_list,plan_date,rent,corpus,interest,rent_adjust,plan_status,dbo.bb_getPunishInterest_item(contract_id,rent_list,"+getSystemDate(1)+") as fx from fund_rent_plan where contract_id='"+contract_id+"' order by rent_list";
	...
	while(rs.next()){
		...
		fx = formatNumberStr(getDBStr(rs.getString("fx")),"#,##0.00");
