...
if( searchFld.equals("项目编号") ) {
	searchFld_tmp = "vi_mproj_guarantee_info.proj_id";
}else if( searchFld.equals("合同编号") ) {
	searchFld_tmp = "vi_mproj_guarantee_info.contract_id";
}else if( searchFld.equals("授信总额度") ) {
	searchFld_tmp = "vi_mproj_guarantee_info.lineof_credit";
}else{
	searchFld_tmp = "";
}
...
String sqlstr="select *,dengjiren=dbo.GETUSERNAME(creator),xiugairen=dbo.GETUSERNAME(modificator) from vi_mproj_guarantee_info"+wherestr;
//String sqlstr = "selectdengjiren=dbo.GETUSERNAME(contract_guarantee_equip.creator),xiugairen=dbo.GETUSERNAME(contract_guarantee_equip.modificator), contract_guarantee_equip.id,contract_guarantee_equip.eqip_name,contract_guarantee_equip.registraction_authority,contract_guarantee_equip.total_price,contract_guarantee_equip.zheng_hao,contract_guarantee_equip.ownership_document,contract_guarantee_equip.equip_guarantee_type,proj_info.project_name,contract_info.contract_id from contract_guarantee_equip left join contract_info on contract_guarantee_equip.contract_id=contract_info.contract_id left join proj_info on contract_info.proj_id=proj_info.proj_id" + wherestr; 
System.out.println("&*"+sqlstr);
%>
<body onLoad="public_onload(0);" style="border:1px solid #8DB2E3;overflow:auto" >
<form action="jsbzj_list.jsp" name="dataNav" onSubmit="return goPage()">
  <!--标题开始-->
  <table border="0" width="100%" cellspacing="0" cellpadding="0" height="25">
    <tr class="tree_title_txt">
      <td nowrap width="100%" class="tree_title_txt" valign="middle" id="cwCellTopTitTxt">主合同结算保证金管理 &gt;结算保证金增加 </td>
    </tr>
  </table>
  <!--标题结束-->
  <!--副标题和操作区开始-->
    ...
    <tr class="maintab">
      <td align="left" colspan="2">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" >
          <tr class="maintab">
            <td align="left">
				&nbsp;按&nbsp;<select name="searchFld"><script>w(mSetOpt("<%= searchFld %>","|合同编号|项目编号|授信总额度"));</script></select>
    ...
    <tr class="maintab">
      <td align="left" width="1%"><!--操作按钮开始-->
        <table border="0" cellspacing="0" cellpadding="0" >
			<tr class="maintab">
				<%if(right.CheckRight("jsbzj_add",dqczy)>0){%><td><a href="#" accesskey="n" onClick="dataHander('add','jsbzj_add.jsp',dataNav.itemselect);"><img   src="../../images/sbtn_new.gif" alt="新增(Alt+N)" align="absmiddle"></a></td><%}%>
				<%if(right.CheckRight("jsbzj_mod",dqczy)>0){%><td><a href="#" accesskey="m" onClick="dataHander('mod','jsbzj_mod.jsp?id=',dataNav.itemselect);"><img   src="../../images/sbtn_mod.gif" alt="修改(Alt+M)" align="absmiddle" ></a></td><%}%>
				<%if(right.CheckRight("jsbzj_del",dqczy)>0){%><td><a href="#" accesskey="d" onClick="dataHander('del','jsbzj_del.jsp?id=',dataNav.itemselect);"><img src="../../images/sbtn_del.gif" alt="删除(Alt+D)" align="absmiddle" ></a></td><%}%>
		    </tr>
        </table>
        <!--操作按钮结束--></td>
...
%>
      ...
      <tr>
       ...
       <td align="center"><a href="jsbzj.jsp?id=<%=getDBStr( rs.getString("id") )  %>" target="_blank"><%=getDBStr( rs.getString("id") )  %></a></td>
       <td align="center"><%= getDBStr( rs.getString("proj_id") ) %></td>
       <td align="center"><%= getDBStr( rs.getString("contract_id") ) %></td>
        <td align="center"><%= getDBStr( rs.getString("sign_date") ) %></td>
