...
<%
String wherestr=" and cust_name like '%保险%'";

String filtervalue=getStr(request.getParameter("fv"));
if ((filtervalue!=null) && (!filtervalue.equals("")))
{
	wherestr+=" and cust_name like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="pay_obj.jsp">
...
<%
String sqlstr="select * from vi_cust_all_info where 1=1 "+wherestr+" order by cust_id";//name-name 
System.out.println("===="+sqlstr);
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('form1.pay_obj_name','form1.pay_obj');" style="height:80%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("cust_id"))%>" returnlist="<%=getDBStr(rs.getString("cust_name"))%>">
	<%=getDBStr(rs.getString("cust_name"))%>--><%=getDBStr(rs.getString("cust_category"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('form1.pay_obj_name','form1.pay_obj');">  
