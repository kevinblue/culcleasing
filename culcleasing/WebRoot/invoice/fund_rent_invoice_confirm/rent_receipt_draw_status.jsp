...
<script type="text/javascript">
	...
	function FundReceiptConfirm(){
		   ...
		   document.getElementById('invoice_statues').value = "导出";
		  
		   
			dataNav.action="rent_receipt_confirm_save.jsp";
			dataNav.target="_blank";
			dataNav.submit();
			dataNav.action="rent_receipt_confirm.jsp";
	...
	function FundReceiptBack(){
		   ...
		   document.getElementById('rent_type_list').value = rent_type_list;
		   document.getElementById('rent_list_list').value = rent_list_list;
		   document.getElementById('invoice_statues').value = "退回";
		  var message="";
		  $.ajax({  
					  ...
					  data: {'plid_list':plid_list,'rent_list_list':rent_list_list,'invoice_flag':'rent_receipt'},  
...
<form action="rent_receipt_draw_status.jsp" name="dataNav" onSubmit="return goPage()"  confirm="false" method="post">
...
countSql = "select count(proj_id) as amount from vi_rent_receipt_draw_status where 1=1 "+wherestr;

//导出类型2--数据导出
String expsqlstr = "select '' 单据编号,'' 单据日期,'' 发票号,'' 发票日期, cust_name 客户名称,'第'+convert(varchar(20),rent_list)+'期租金'+rent_type+'(融资租赁)'  商品,'1' 数量,plan_money   单价,plan_money 金额,invoice_remark 备注,'' 收款人,'0' 状态 from vi_rent_receipt_draw_status where 1=1 "+wherestr; 
System.out.println("vvvvvvvvvvvvv"+expsqlstr);

String updSql="select contract_id,fee_name,fee_num,id from vi_rent_receipt_draw_status where 1=1 "+wherestr;
%>
<!--可折叠查询开始-->
...
<td>领取状态:
	<select name="status" style="width:105">
		<script type="text/javascript">
			w(mSetOpt('<%=status%>',"|已领取|未领取","|已领取|未领取"));
		</script>
	 </select>
</td>
...
sqlstr = "select top "+ intPageSize +" * from vi_rent_receipt_draw_status  where id not in( ";
sqlstr += " select top "+ (intPage-1)*intPageSize +" id from vi_rent_receipt_draw_status where 1=1 "+wherestr+" order by id ) "+wherestr ;
sqlstr +=" order by id ";
