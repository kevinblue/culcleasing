...
if ( stype.equals("del") ){ 
	...
	for(int i = 0; i < priIdS.length; i++) {
		sqlstr = "Delete from invoice_draw_detail where pri_id='"+priIdS[i]+"' and invoice_type='资金' ";
		flag += db.executeUpdate(sqlstr);
	}
	
	//更新apply_info表
	sqlstr="Update invoice_draw_info set amount_t=isnull(amount_t,0)-"+priIdS.length+",amount_zj=isnull(amount_zj,0)-"+priIdS.length+" where glide_id ='"+glide_id+"' ";
	flag += db.executeUpdate(sqlstr);
	...
	for(int i = 0; i < priIdS.length; i++) {
		...
		sqlstr += "Select contract_id,'"+glide_id+"',id,'资金','0','"+dqczy+"','"+systemDate+"' from vi_func_fund_manage_draw_new where id='"+priIdS[i]+"'";
		flag += db.executeUpdate(sqlstr);
	}
