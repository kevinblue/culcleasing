...
<%
	...
	String cfri_id_list = getStr(request.getParameter("cfri_id_list"));
	...
	String [] cfri_arr = cfri_id_list.split("#");
	...
	for(int i=0;i<id_arr.length;i++){
		...
		if("后开，租金总额发票".equals(invoice_type_arr[i])){
			sql = " insert  into  [dbo].[rent_invoice_info](begin_id,contract_fund_rent_income,tax_type,"+
				   ...
				   sql+="null";
				   sql+=")";
		}else if("后开，本金收据，利息发票".equals(invoice_type_arr[i])){
			System.out.println("后开，本金收据，利息发票");
			sql = " insert  into  [dbo].[rent_invoice_info](begin_id,contract_fund_rent_income,tax_type,"+
						 " tax_type_invoice,invoice_status,bill_type,rent_type,invoice_money,creator,create_date,modificator,modify_date) "+
						 " values(";
				   sql+="'"+begin_arr[i] +"',";
			...
			sql += " insert  into  [dbo].[rent_invoice_info](begin_id,contract_fund_rent_income,tax_type,"+
				   ...
				   sql+="null";
				   sql+=")";
				   	System.out.println("后开，本金收据，利息发票:"+sql);
			
		}else if("后开，本利分开".equals(invoice_type_arr[i])){
			sql = " insert  into  [dbo].[rent_invoice_info](begin_id,contract_fund_rent_income,tax_type,"+
						 " tax_type_invoice,invoice_status,bill_type,rent_type,invoice_money,creator,create_date,modificator,modify_date) "+
						 " values(";
				   sql+="'"+begin_arr[i] +"',";
		...
		}else	if("后开，本金收据，利息发票，另加总额收据".equals(invoice_type_arr[i])){
			sql = " insert  into  [dbo].[rent_invoice_info](begin_id,contract_fund_rent_income,tax_type,"+
						 " tax_type_invoice,invoice_status,bill_type,rent_type,invoice_money,creator,create_date,modificator,modify_date) "+
						 " values(";
				   sql+="'"+begin_arr[i] +"',";
