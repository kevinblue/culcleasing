...
String invoice_date_str = " ";
	...
	if((kj==null || "".equals(kj))){//非全选操作
		for(int i=0;i<item.length;i++){
			...
			if(rs.next()){
				...
				sqlstr="insert into invoice_penalty_detail ( begin_id,rent_list,pri_id,invoice_is,invoice_remark,invoice_date,creator,create_date,modificator,modify_date)values('"+begin_id+"','"+rent_list+"',"+pri_id+",'"+invoice_is+"','"+invoice_remark+"','"+("是".equals(invoice_is)?invoice_date:"")+"','"+dqczy+"',getdate(),'"+dqczy+"',getdate())";
	...
	LogWriter.logDebug(request, "罚息发票管理确认");
	msg = "罚息发票管理确认";
}
