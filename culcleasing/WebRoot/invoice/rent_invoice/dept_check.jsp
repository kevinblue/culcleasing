...
function ListSelect(dobj,vobj,dobj1,vobj1){//05.04.27
...
<td class="tree_title_txt"   valign="middle" align="center">选择落实人</td>
</tr>
...
{
	wherestr=" and (parent_deptname+dept_name) like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="dept_check.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>名称: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr="SELECT * FROM v_select_base_department where 1=1 "+wherestr+" order by order_field";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('dataNav.parent_dpetname','dataNav.parent_deptnameVal','dataNav.dept_name','dataNav.dept_nameVal');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("parent_deptname"))%>" returnlist="<%=getDBStr(rs.getString("parent_deptname"))%>" value1="<%=getDBStr(rs.getString("dept_name"))%>" returnlist1="<%=getDBStr(rs.getString("dept_name"))%>">
	<%=getDBStr(rs.getString("parent_deptname"))%>--<%=getDBStr(rs.getString("dept_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('dataNav.parent_dpetname','dataNav.parent_deptnameVal','dataNav.dept_name','dataNav.dept_nameVal');">  
