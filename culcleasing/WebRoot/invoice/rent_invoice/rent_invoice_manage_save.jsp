...
if("updStatus".equals(type)){
	...
	if( (invoice_date!=null && !"".equals(invoice_date))  ){
		...
		while(rs1.next()){
			...
			if(rs.next()){
				sqlstr = "update invoice_rent_detail set invoice_date='"+invoice_date+"', invoice_remark='"+invoice_remark+"',modificator='"+dqczy+"',modify_date=getdate() ";
				sqlstr+= " where begin_id='"+begin_id+"' and rent_list='"+rent_list+"'";
				
				flag += db.executeUpdate(sqlstr);
			}else{
				sqlstr = "insert into invoice_rent_detail( begin_id,rent_list,invoice_is,invoice_normal,invoice_remark,invoice_date,creator,create_date,modificator,modify_date) ";
				sqlstr+="values('"+begin_id+"','"+rent_list+"','null',null,'"+invoice_remark+"','"+invoice_date+"','"+dqczy+"',getdate(),'"+dqczy+"',getdate())";
...
*/
	...
	if( (kj!=null && !"".equals(kj))  ){
		...
		while(rs1.next()){
			...
			rs=db.executeQuery(sqlstr);
			if(rs.next()){
				sqlstr = "update invoice_rent_detail set invoice_is='"+invoice_is+"' ";
				...
				sqlstr = "insert into invoice_rent_detail( begin_id,rent_list,invoice_is,invoice_normal,invoice_remark,invoice_date,creator,create_date,modificator,modify_date) ";
				sqlstr+="values('"+begin_id+"','"+rent_list+"','"+invoice_is+"',null,'"+invoice_remark+"','"+invoice_date+"','"+dqczy+"',getdate(),'"+dqczy+"',getdate())";
				System.out.println("aaabb11111:"+sqlstr);
				flag = db.executeUpdate(sqlstr);
			}
		}
	...
	if( (zckj!=null && !"".equals(zckj)) ){
		...
		while(rs1.next()){
			...
			rs=db.executeQuery(sqlstr);
			if(rs.next()){
				sqlstr = "update invoice_rent_detail set invoice_remark='"+invoice_remark+"'";
				sqlstr+= ",modificator='"+dqczy+"',modify_date=getdate(),invoice_normal='"+invoice_normal+"'"+invoice_date_str;
				sqlstr+= " where begin_id='"+begin_id+"' and rent_list='"+rent_list+"'";
				flag += db.executeUpdate(sqlstr);
			}else{
				sqlstr = "insert into invoice_rent_detail( begin_id,rent_list,invoice_is,invoice_normal,invoice_remark,invoice_date,creator,create_date,modificator,modify_date) ";
				sqlstr+=" values('"+begin_id+"','"+rent_list+"',null,'"+invoice_normal+"','"+invoice_remark+"','"+invoice_date+"','"+dqczy+"',getdate(),'"+dqczy+"',getdate())";
	...
	if((kj==null || "".equals(kj)) && (zckj==null || "".equals(zckj))){//非全选操作
		for(int i=0;i<item.length;i++){
			...
			if(rs.next()){
				...
				sqlstr="insert into invoice_rent_detail (begin_id,rent_list,invoice_is,invoice_normal,invoice_remark,invoice_date,creator,create_date,modificator,modify_date) values('"+begin_id+"','"+rent_list+"','"+invoice_is+"','"+invoice_normal+"','"+invoice_remark+"','"+("是".equals(invoice_is)?invoice_date:"")+"','"+dqczy+"',getdate(),'"+dqczy+"',getdate())";
