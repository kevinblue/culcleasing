...
function TWebCalendar() //初始化日历的设置
{
    ...
    this.darkColor  = "#8DB2E3";    //控件的暗色
    this.lightColor = "#FFFFFF";    //控件的亮色
    this.btnBgColor = "#FFFFFf";    //控件的按钮背景色
    this.wordColor  = "#15428B";    //控件的文字颜色
    this.wordDark   = "#DCDCDC";    //控件的暗文字颜色
    this.dayBgColor = "#F4F9FF";    //日期数字背景色
    this.todayColor = "#0D3793";    //今天在日历上的标示背景色
    this.DarkBorder = "#F4F9FF";    //日期显示的立体表达色
}   
...
function show_calendar(str){
  var key=str.indexOf(".")!=-1?str.substring(str.indexOf(".")+1):str;
  var tag = document.getElementById(key);	  
  return  calendar(event,tag);
}	
