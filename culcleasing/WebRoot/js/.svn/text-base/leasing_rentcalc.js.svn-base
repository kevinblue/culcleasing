...
$(document).ready(function(){
	...
	$(":input[name='year_rate']").blur(function(){
		//年利率
		if( isNaN($(this).val()) || $(this).val()<0 || !/^[0-9]+.?[0-9]*$/.test($(this).val()) ){ // /^[0-9]+.?[0-9]*$/
			alert("年利率填写格式不正确");
			$(this).val(0);
			//$(this).focus();
		}
	...
	$("#tax_type").change(function(){
		//alert($(this).val());
		if($(this).val()=="营业税"){
			$("select[name='tax_type_invoice']").val("无");
		}
		else{
			$("select[name='tax_type_invoice']").val("增值税普通发票");
		}
	});
...
//	var insure_money = document.getElementsByName("insure_money")[0].value;//保费金额
	...
	if((lease_capital_value == null || lease_capital_value == '') || (caution_money_value == null || caution_money_value == '')){
		...
		//保费金额根据 支付方式 决定不同的净融资额 2012-4-5
		...
		document.getElementsByName("actual_fund")[0].value = round2places(lease_amt_value);
