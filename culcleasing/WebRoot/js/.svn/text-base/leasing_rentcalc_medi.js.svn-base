...
$(document).ready(function(){
	...
	//是否保底	jaffe 2012-3-9取消
	/**
	$(":radio[name='is_floor']").click(function(){
		var is_floor = $(":radio[name='is_floor']:checked").val();
		if(is_floor=="是"){
			$(":input[name='floor_rent']").removeAttr("readonly");//取消
		}else{
			$(":input[name='floor_rent']").attr("readonly","readonly");//不启用控件
		}
	});
	**/
