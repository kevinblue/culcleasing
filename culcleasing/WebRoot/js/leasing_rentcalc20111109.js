...
$(document).ready(function(){
	...
	$(":input[name='year_rate']").blur(function(){
		//年利率
		if( $(this).val()<=0 || !/^[0-9]+.?[0-9]*$/.test($(this).val()) ){ // /^[0-9]+.?[0-9]*$/
			alert("年利率填写格式不正确");
			$(this).val(0);
			$(this).focus();
		}
...
function checkDataInvalid(totalM,usedM){
	var equipAmt = $("#equip_amt").val();
	//校验金额填写正确
	if(isNaN(equipAmt)){
		alert("请输入正确金额！");
		//$("#equip_amt").val("0");
	}else{
		var uM = FloatSub(totalM,FloatAdd(equipAmt,usedM));
		if (uM>=0){
...
function alertChange(){
	...
	//利率浮动类型 "按央行利率浮动比率|按央行利率加点|隐含利率|保持不变","0|1|2|3"
...
function changeOne(){
	...
	var div_rate = document.getElementById('div_rate');
	if(rate_float_type == '1' || rate_float_type == '3'){
