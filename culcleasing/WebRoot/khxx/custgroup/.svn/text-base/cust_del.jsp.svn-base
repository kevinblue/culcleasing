...
int canmod=0;
	...
	String sqlstr = "SELECT cust_group_company.cust_id,vi_cust_all_info.cust_name FROM cust_group_company LEFT OUTER JOIN vi_cust_all_info ON cust_group_company.cust_id = vi_cust_all_info.cust_id where cust_group_company.cust_id='" + czid + "'"; 
...
<input type="hidden" name="cust_id" value="<%= czid %>">
