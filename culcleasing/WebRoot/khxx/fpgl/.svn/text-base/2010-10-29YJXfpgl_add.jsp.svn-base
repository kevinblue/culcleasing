...
String sql="select * from vi_invoice " + where_str+" and vi_invoice.id not in(select plan_id from proj_invoice_detail where proj_invoice_detail.proj_invoice_id in(select id from proj_invoice)) order by contract_id";
...
<table border="0" width="100%" cellspacing="5" cellpadding="0">
  ...
  <tr class="maintab">
				<td align="left" colspan="2">               
					&nbsp;按&nbsp;<select class="text" name="searchFld"><script>w(mSetOpt("<%= searchFld %>","|合同编号|承租人|租金"));</script></select>&nbsp;查询&nbsp;<input class="text" name="searchKey" accesskey="s" type="text" size="15" value="<%= searchKey %>">
