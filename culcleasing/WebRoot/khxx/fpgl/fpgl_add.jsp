...
String sql="select vi_invoice.*,isnull(hct.other_interest,0.00) other_interest from vi_invoice left join handling_charge_temp hct on vi_invoice.contract_id=hct.contract_id and vi_invoice.rent_list=hct.plan_list " + where_str+" and isnull(vi_invoice.is_fp,0)<>'是' order by vi_invoice.contract_id";
...
onClick="OpenDataWindow('','','','','承租人','vi_cust_all_info_t','cust_name','cust_id','cust_name','cust_name','asc','search.cust_name','search.cust_name');"><span class="biTian">*</span>
					...
					&nbsp;按&nbsp;<select class="text" name="searchFld"><script>w(mSetOpt("<%= searchFld %>","|合同编号|租金金额"));</script></select>
...
<tr class="maintab">
 ...
 <td>
	 ...
	 <td scope="col">发票类型:</td>
    <td> 
   <select class="text" name="invoice_method" id="invoice_method" onchange="makeValue();" ><script>w(mSetOpt("租金","|租金|利息"));</script></select><span class="biTian">*</span>
	</td>
</tr>
