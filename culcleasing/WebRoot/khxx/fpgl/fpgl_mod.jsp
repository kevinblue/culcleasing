...
String interest="";
 ...
 rs = db.executeQuery(sqlstr);
	...
	invoice_name=getDBStr(rs.getString("invoice_name"));
	invoice_method=getDBStr(rs.getString("invoice_method"));
...
<table class="tab_table_title" border="0" cellspacing="0" cellpadding="1" width="100%">
  <tr>
    <td scope="row">发票总额：</td>
    <td><input  class="text"  type="text" value="<%=formatNumberStr(rs.getString("invoice_total"),"#,##0.00")%>" readonly="readonly"></td>
   <td scope="row">发票抬头：<input class="text" type="text" name="invoice_name" readonly="readonly" value="<%=invoice_name %>" Require="true"  maxlength="40" ></td>
<td scope="row">开据时间:&nbsp;&nbsp;</td><td>
        <input class="text" type="text" name="invoice_date" value="<%=invoice_date %>"  Require="true" maxlength="40" readonly>
 <img onClick="openCalendar(invoice_date);return false;" style="cursor:pointer; " src="../../images/fdmo_63.gif" border="0" align="absmiddle"> <span class="biTian">*</span></td>
  </td>
   	<td>备注:<td>
    <td rowspan="2">
    <textarea name="memo" style="width: 300;height: 70"><%=memo %></textarea></td>
  </tr>
  <tr>
       <td scope="row">发票类型：</td>
       <td>
        
       <input class="text" type="text" name="invoice_method" value="<%=invoice_method %>"  Require="true" maxlength="40" readonly> 
     <%--  <select class="text" name="invoice_method" id="invoice_method" ><script>w(mSetOpt("<%=invoice_method %>","|租金|利息"));</script></select>--%>
       </td>
         <td scope="row">发&nbsp;票&nbsp;号：&nbsp;&nbsp;<input class="text" type="text" name="invoice_number" value="<%=invoice_number %>" Require="true"  maxlength="40"><span class="biTian">*</span></td>
       <td></td>
 	  </tr>
  <tr>
...
<table border="0" style="border-collapse:collapse;" align="center"
		...
		<th>租息</th>
						...
						String sqls = "select vi_invoice.*,hct.other_interest from proj_invoice left join proj_invoice_detail on proj_invoice.id=proj_invoice_detail.proj_invoice_id left join vi_invoice on vi_invoice.contract_id=proj_invoice_detail.contract_id and vi_invoice.rent_list= proj_invoice_detail.rent_list  left join handling_charge_temp hct on vi_invoice.contract_id=hct.contract_id and vi_invoice.rent_list=hct.plan_list "+where_str+"  and  proj_invoice.id="+id+"";
