...
<form name="form1" method="post" action="fund_fpgl_save.jsp" onSubmit="return Validator.Validate(this,3);">
  ...
  <tr>
   <td scope="row">发票号：</td>
    <td>
      ...
      <td scope="row">发票抬头：</td>
    <td>
        <input class="text" type="text" name="invoice_name" readonly="readonly" value="<%=invoice_name %>" Require="true"  maxlength="40" ></td>
...
<table border="0" style="border-collapse:collapse;" align="center"
	    ...
	    <th>计划收款日期</th>
						...
						String sqls = "select vi_fund_invoice.* from proj_invoice left join proj_invoice_detail on proj_invoice.id=proj_invoice_detail.proj_invoice_id left join vi_fund_invoice on vi_fund_invoice.id=proj_invoice_detail.plan_id "+where_str+"  and  proj_invoice.id="+id+"";
...
function isExport() {
	if (confirm("是否确认导出Excel!")) {
		...
		document.getElementById("where_str").value="<%=wherestr%>";
  		form1.submit();
		form1.action="fund_fpgl_mod.jsp";
	}
	return false;
}
