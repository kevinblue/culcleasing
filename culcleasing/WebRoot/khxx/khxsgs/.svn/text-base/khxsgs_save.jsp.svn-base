...
if ( stype.equals("mod") ){      //修改操作
	...
	sqlstr = "update cust_member_company set rank='" + rank + "',asset_size='"+asset_size+"',profit_year='"+profit_year+"',reg_number='" + reg_number +"',stake='" + stake +"',biz_scope_primary='" + biz_scope_primary + "',asset_liability='"+asset_liability+"',memo='"+memo+"',modificator='" + dqczy  + "',modify_date='" + systemDate + "' where id='" + czid + "'";
...
if ( stype.equals("del") ){         //删除操作
	...
	//String czid = getStr( request.getParameter("id") );
	sqlstr = "delete from cust_member_company where id='" + czid + "'";
	db.executeUpdate(sqlstr); 
%>
	<script>
		window.close();
