...
<td class="tree_title_txt"   valign="middle" align="center">选择项目</td>
</tr>
...
String wherestr=" and cust_id='"+cust_id+"'";
...
%>
<form name="searchbar" action="proj_info.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>项目名称: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr="select proj_id,project_name from vi_contract_info where 1=1 "+wherestr+" order by proj_id";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('form1.res_project','form1.project_name');" style="height:80%;">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("project_name"))%>" returnlist="<%=getDBStr(rs.getString("proj_id"))%>">
	<%=getDBStr(rs.getString("proj_id"))%> -- <%=getDBStr(rs.getString("project_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('form1.res_project','form1.project_name');">  
