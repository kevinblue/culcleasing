				...
				for(var i=0; i<selNames.length; i++){
					if(selNames[i].checked == true){
						if(selNames[i].value==""||selNames[i].value=="."){
						   ...
						   val += "|"+selNames[i].attributes["plan_id"].nodeValue+";";
						  //alert(idstr);
						}						
			        }					    						
				}
...
String wherestr = "where plan_status='未核销' ";
String searchFld = getStr( request.getParameter("searchFld") );
String searchKey = getStr( request.getParameter("searchKey") );
if(searchFld.equals("供应商")&&!searchKey.equals("")){
   wherestr += " and cust_name like '%"+searchKey+"%'";
}else if(searchFld.equals("合同号")&&!searchKey.equals("")){
...
String sqlstr = "select id,contract_id,pay_cust,cust_name,client_bank,client_accnumber,plan_money,";
	   sqlstr+= "plan_date from vi_phone_charge_plan "+wherestr;
  ...
  <!--副标题和操作区开始-->
	...
	<tr class="maintab">
				<td align="left" colspan="2">               
					&nbsp;按&nbsp;<select name="searchFld"><script>w(mSetOpt("<%= searchFld %>","|供应商|合同号"));</script></select>&nbsp;查询&nbsp;<input name="searchKey" accesskey="s" type="text" size="15" value="<%= searchKey %>">
...
%>
      <tr>
        <td align="center" nowrap><input type="checkbox" name="ssid" plan_id="<%=getDBStr( rs.getString("id") ) %>" value="<%=getDBStr( rs.getString("pay_cust") ) %>">&nbsp;<%=intPageSize*(intPage-1)+i+1 %></td>
		<td align="center" nowrap><%=getDBStr( rs.getString("cust_name") )  %></td>
		<td align="center" nowrap><%=getDBStr( rs.getString("contract_id") )  %></td>
		<td align="center" nowrap><%=getDBStr( rs.getString("client_bank") )  %></td>
		<td align="center" nowrap><%=getDBStr( rs.getString("client_accnumber") )  %></td>
		<td align="center" nowrap><%=formatNumberStr( rs.getString("plan_money"),"#,##0.00") %></td>
