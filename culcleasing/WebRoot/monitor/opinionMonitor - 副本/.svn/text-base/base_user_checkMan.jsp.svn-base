...
<td class="tree_title_txt"   valign="middle" align="center">选择意见核实人员</td>
</tr>
...
%>
<form name="searchbar" action="base_user_checkMan.jsp">
...
<%
String sqlstr="SELECT * FROM base_user where status=1"+wherestr+" order by name";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('form1.check_man','form1.id2');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("id"))%>" returnlist="<%=getDBStr(rs.getString("name"))%>">
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('form1.check_man','form1.id2');">  
