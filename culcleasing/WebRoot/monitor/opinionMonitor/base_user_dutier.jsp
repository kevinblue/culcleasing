...
%>
<form name="searchbar" action="base_user_dutier.jsp">
...
<%
String sqlstr="SELECT bd.dept_name as dept_name,bd.id as dept_id,* FROM base_user bu LEFT JOIN base_department bd ";
sqlstr+=" ON bd.id=bu.department where bu.status=1 "+wherestr+" order by bu.name";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('form1.dutier','form1.id1','form1.operation_dept','form1.dept_id');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("id"))%>" returnlist="<%=getDBStr(rs.getString("name"))%>" value1="<%=getDBStr(rs.getString("dept_id"))%>" returnlist1="<%=getDBStr(rs.getString("dept_name"))%>">
	<%=getDBStr(rs.getString("name"))%>--<%=getDBStr(rs.getString("dept_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('form1.dutier','form1.id1','form1.operation_dept','form1.dept_id');">  
