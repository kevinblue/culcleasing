...
if ( stype.equals("add") ){        //添加操作
	//insert
	sqlstr = "insert into opinion_execution(proj_id,raiser,raiser_date,idea,operation_dept,flow,status,creator,create_date,modificator,modify_date)";
	sqlstr += " values('"+proj_id+"','"+raiser+"','"+raiser_date+"','"+idea+"','"+operation_dept+"','"+flow+"',0,'"+dqczy+"','"+datestr+"','"+dqczy+"','"+datestr+"')";
	...
	msg = "修改项目执行意见";
}else if ("sucPre".equals(stype)){//完成
	String kid =  getStr( request.getParameter("kid") );
