...
//先判断数据库里是否存在
String Sid="";
String sql = "select id from base_user_respons where base_user_id='"+id+"'";
...
if(Sid!=""){
	sqlstr = "update base_user_respons set respon_provice_id='"+respon_provice_id+"',respon_provice_title='"+respon_provice_title+"',";
	sqlstr+= "modificator='"+dqczy+ "',modify_date='"+datestr+"' where base_user_id='"+id+"'";
	LogWriter.logDebug(request,"11111111111111111我需要的up-sqlstr:"+sqlstr);
	flag = db.executeUpdate(sqlstr);
}else{
	sqlstr ="insert into base_user_respons (base_user_name,base_user_id,respon_provice_id,respon_provice_title,creator,create_date,modificator,modify_date)";
