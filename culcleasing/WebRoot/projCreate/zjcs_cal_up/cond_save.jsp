...
<%
	...
	String before_interest_type = getZeroStr(getStr(request.getParameter("before_interest_type")));//租前息类型-是否算本
	...
	String irr = getZeroStr(getStr(request.getParameter("irr")));//实际IRR，财务填写 20111018新增
	...
	String insur_pay_type=getStr(request.getParameter("insure_pay_type"));
	//2012-9-10 zhangqi新增税种
	...
	int res = 0;
	//1.2==创建对象，属性赋值
	ConditionBean conditionBean = new ConditionBean(doc_id,proj_id,equip_amt,currency,lease_money,
	...
	res = ConditionService.saveConditionBeanIntoTemp(conditionBean, saveType);

	res +=ConditionService1.flowUpdateTaxtype(conditionBean);
	...
	if("RentCalcType8".equals(settle_method) || "RentCalcType9".equals(settle_method) || "RentCalcType10".equals(settle_method)){
		...
		RentInfoBox rentInfoBox = RentPlanService.calcRentPlan(conditionBean);
			
		//================4.保存租金计划、现金流================
		res += RentInfoDBOperation.operProjRentInfoBox2DB(rentInfoBox, conditionBean);//保存
		LogWriter.logDebug(request, "项目立项 - 租金测算完成，保存租金计划、现金流成功！");
	}
