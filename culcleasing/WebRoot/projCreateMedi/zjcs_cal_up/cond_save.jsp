...
<%
	...
	if( "是".equals(is_floor) ){//判断是否保底
		RentInfoBox rentInfoBox = RentPlanService.calcMediProjRentPlan(conditionMediBean);
		
		//================4.保存保底 租金计划、现金流================
		flag += RentInfoDBOperation.operMediProjRentInfoBox2DB(rentInfoBox, conditionMediBean);//保存
			
		LogWriter.logDebug(request, "起租租金测算完成，保存租金计划、现金流成功！");
		
	}else{ //非保底无需要操作
		String sqlStr = "Delete from fund_rent_plan_proj_bd_medi_temp where proj_id='"+proj_id+"' and doc_id='"+doc_id+"'";
		db.executeUpdate(sqlStr);
		System.out.println("非保底租金项目,删除租金计划临时数据："+sqlStr);
	}
