...
sqlstr+= " from proj_condition_medi_temp where proj_id='"+proj_id+"' and doc_id='"+doc_id+"'";
/* 
名称	收支情况
...
}else if("15".equals(fee_type)){//保证金  - 涉及退款
	fee_type_name = "保证金";
...
if(outFlag==0){//没有超出则插入数据库						  
	sqlstr = "insert into proj_fund_fund_charge_plan_temp(payment_id,measure_id,proj_id,pay_type,fee_type,fee_name,fee_num,plan_date,plan_status,curr_plan_money,plan_money,currency,pay_obj,";
	sqlstr+=" pay_bank_name,pay_bank_no,plan_bank_name,plan_bank_no,pay_way,fpnote,creator,create_date,modificator,modify_date)";
	// 付款 -- 设备款
	sqlstr+=" select '"+CommonTool.getUUID()+"','"+doc_id+"','"+proj_id+"','"+pay_type+"','"+fee_type+"','"+fee_name+"','"+(fee_num+1)+"','"+plan_date+"','未核销','"+plan_money+"','"+plan_money+"','"+currency+"','"+pay_obj+"',";
