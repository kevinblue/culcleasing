...
int flag = 0;
	if("add".equals(saveType)){//新增计算过的保证金补帖
		//增加基础表记录
		sqlstr = "delete from begin_caution_info_medi where begin_id='"+begin_id+"'";
		sqlstr+="insert into begin_caution_info_medi(begin_id,caution_money,caution_term,flow_date,create_date,creator) values ";
		...
		//先删除旧的数据
		sqlstr = "delete from begin_caution_subsidy_medi where begin_id='"+begin_id+"'";
		db.executeUpdate(sqlstr);
		sqlstr="";
		for(int i=1;i<=Integer.parseInt(interest_num);i++){
			sqlstr+=" insert into begin_caution_subsidy_medi(begin_id,subsidy_money,subsidy_list,create_date,creator) values ";
