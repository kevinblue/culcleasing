...
System.out.println("输出币种"+currency);
...
int flag = 0;
ResultSet rs = null;
 sqlstr="select name from ifelc_conf_dictionary where title='"+currency+"'";
 rs=db.executeQuery(sqlstr);
 if(rs.next()){
 	currency=rs.getString("name");
 }
//1查询交易结构临时表的资金计划
...
sqlstr+= " from proj_condition_temp where contract_id='"+contract_id+"' and doc_id='"+doc_id+"'";
/* 
名称	收支情况
1-设备金额	支  equip_amt
...
if(outFlag==0){//没有超出则插入数据库
	...
	sqlstr = "update contract_fund_fund_charge_plan_temp set fee_name='"+fee_name+"',pay_obj='"+pay_obj+"',plan_date='"+plan_date+"',curr_plan_money='"+plan_money+"',plan_money='"+plan_money+"',currency='"+currency+"',pay_bank_name='"+pay_bank_name+"',pay_bank_no='"+pay_bank_no+"',";
