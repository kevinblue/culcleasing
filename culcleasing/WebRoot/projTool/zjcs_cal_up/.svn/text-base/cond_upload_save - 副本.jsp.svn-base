...
System.out.println("文件名uploadFileName》》》》》"+uploadFileName);
System.out.println("文件名uploadFileName长度》》》》》"+uploadFileName.length());
...
System.out.println("文件名%%%%uploadFileName======="+conditionBean.getSettle_method());
	   ...
	   if(saObjectFile[j]!=null&&!saObjectFile[j].equals("")){
		  ...
		  if(execlBean.isFlag()){
				...
				**/
				...
				conditionBean.setIrr(obj[7][7]);
				//达标Irr 与是否达标
				conditionBean.setStandardIrr(obj[7][7]);
				conditionBean.setStandardF("是");
				//开票方式
	
				
				System.out.println("测试输出irr"+conditionBean.getIrr());
				//T.3净融资额 - 第12行第7列
				...
				conditionBean.setEnd_date(obj[11+Integer.parseInt(conditionBean.getLease_term())][0]);//结束日期
				...
				conditionBean.setIncome_number_year( MathExtend.divide( conditionBean.getLease_term(), conditionBean.getIncome_number() ).substring(0,1) );
				...
				**/
				for(int row=12;row<ConvertUtil.parseInt(conditionBean.getLease_term(),12)+12;row++){//遍历多行
