...
String sqluser="select project_name from contract_info where contract_id='" + contract_id+"'";
...
if ( stype.equals("mod") ){      //修改操作	
		sqlstr = "update contract_fund_fund_charge_plan set wh_status ='未同步' ,expect_money='"+expect_money+"' ,expect_put_time='"+expect_put_time+"',wh_modificator='"+wh_modificator+"' ,wh_modify_date='"+wh_modify_date+"' where id='" + id + "'";
		System.out.println("sqlstrsqlstr=="+sqlstr);
		sqlstr1 = "insert into contract_fund_fund_charge_plan_log_info";
		sqlstr1 += " values('"+id+"','"+contract_id+"','"+project_name+"',"+
		"'"+fee_name+"','"+expect_put_time+"','"+expect_money+"','"+wh_modificator+"','"+wh_modify_date+"')";
