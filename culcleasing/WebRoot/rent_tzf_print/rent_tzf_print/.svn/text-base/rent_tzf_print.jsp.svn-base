...
//------------------------------------------------
	...
	if(line_str1.equals("")){
		if(Double.parseDouble(pre_punish)>0){
			line_str1+="第"+pre_rent_list+"期租金 金额为￥0.00元";
			line_str1+="，逾期"+pre_days+"天,该期罚息金额为￥"+formatNumberStr(pre_punish,"#,##0.00");
		}
	}else{
		if(Double.parseDouble(pre_punish)>0){
			line_str1+="，逾期"+pre_days+"天,该期罚息金额为￥"+formatNumberStr(pre_punish,"#,##0.00");
...
obj.SetFontStyle 0,1,0
obj.DrawObject "<%=curr_plan_date.substring(0,4)%>年<%=curr_plan_date.substring(5,7)%>月<%=curr_plan_date.substring(8,10)%>日", "", 0, "", 1, "1,2,3,4", "92,310", "false,2"

obj.SetFontStyle 0,0,0
obj.DrawObject "（应到账日）以前向我公司账户汇入当期款项：", "", 0, "", 1, "1,2,3,4", "192,310", "false,2"
obj.DrawObject "第<%=curr_rent_list%>期租金 金额为 ￥<%=formatNumberStr(curr_rent,"#,##0.00")%>元", "", 0, "", 1, "1,2,3,4", "28,330", "false,2"
...
obj.SetFontStyle 0,1,1
obj.DrawObject "名义留购价￥<%=formatNumberStr(nominalprice,"#,##0.00")%>元", "", 0, "", 1, "1,2,3,4", "70,472", "false,2"
obj.SetFontStyle 0,0,0
...
obj.SetFontStyle 0,1,1
obj.DrawObject "￥<%=formatNumberStr(total_money,"#,##0.00")%>元  ", "", 0, "", 1, "1,2,3,4", "122,532", "false,2"
obj.SetFontStyle 0,0,0
<%}%>
...
obj.DrawObject "1、	依租赁合同第四条规定，贵方若出现逾期则须按日<%=formatNumberStr(String.valueOf(Double.parseDouble(pena_rate)/100),"#,##0.00")%>%支付延付罚息。", "", 0, "", 1, "1,2,3,4", "0,594", "false,2"
...
obj.DrawObject "<%=bad_date.substring(0,4)%>年<%=bad_date.substring(5,7)%>月<%=bad_date.substring(8,10)%>日", "", 0, "", 1, "1,2,3,4", "466,778", "false,2"
...
obj.DrawObject "当期租金 ￥<%=formatNumberStr(curr_rent,"#,##0.00")%>元", "", 0, "", 1, "1,2,3,4", "26,859", "false,2"


'暂时隐去罚息
'obj.DrawObject "当期罚息￥<%=formatNumberStr(c_punish,"#,##0.00")%>元", "", 0, "", 1, "1,2,3,4", "26,878", "false,2"
...
obj.DrawObject "逾期款项 ￥<%=formatNumberStr(total_money_bad,"#,##0.00")%>元", "", 0, "", 1, "1,2,3,4", "26,897", "false,2"

obj.SetFontStyle 0,0,1
<%if(show_flag.equals("0")){%>
obj.DrawObject "留购价 ￥<%=formatNumberStr(nominalprice,"#,##0.00")%>元", "", 0, "", 1, "1,2,3,4", "26,918", "false,2"
<%}else{%>
obj.DrawObject "", "", 0, "", 1, "1,2,3,4", "26,918", "false,2"
<%}%>

obj.SetFontStyle 0,1,0
obj.DrawObject "合计￥<%=formatNumberStr(total_money,"#,##0.00")%>元", "", 0, "", 1, "1,2,3,4", "26,938", "false,2"
obj.SetFontStyle 0,0,0
...
//将租金通知书内容保存到数据库
	...
	notice_content+="（1）贵方应于"+curr_plan_date.substring(0,4)+"年"+curr_plan_date.substring(5,7)+"月"+curr_plan_date.substring(8,10)+"日（应到账日）以前向我公司账户汇入当期款项："+"<br>";
	...
	notice_explanation+="3、	如已全部支付本通知书所列的款项，请忽略本通知书；"+"<br>";
	notice_explanation+="如已部分支付或尚未支付，请按本通知书所提示的内容按时足额支付所列的款项。"+"<br>";
	
	if(show_flag.equals("0")){
		notice_explanation+="4、	该租赁合同约定于"+end_date+"结束。"+"<br>";
	}
