...
ResultSet rs;
ResultSet rs1;
String wherestr = " where 1=1 and dbo.bb_getDeduRent_tzf(contract_condition.contract_id,fund_rent_plan.rent_list)>0";
...
while(rs.next()){
	...
	if(Double.parseDouble(pre_rent)>0){
		line_str1="第"+pre_rent_list+"期租金 金额为￥"+formatNumberStr(pre_rent,"#,##0.00")+"元，逾期"+pre_days+"天,计罚息金额为￥"+formatNumberStr(pre_punish,"#,##0.00");
	}
	//2
	...
	if(Double.parseDouble(pre_pre_rent)>0){
		line_str3="第"+pre_pre_rent_list+"期租金 金额为￥"+formatNumberStr(pre_pre_rent,"#,##0.00")+"元，逾期"+pre_pre_days+"天,计罚息金额为￥"+formatNumberStr(pre_pre_punish,"#,##0.00");
	}
	
	//3
	
	pre_pre_pre_rent=String.valueOf(Double.parseDouble(curr_badRent)-Double.parseDouble(pre_rent)-Double.parseDouble(pre_pre_rent));
	//System.out.println("pre_pre_pre_rent=================================="+pre_pre_pre_rent);
	
	//System.out.println("66==================================");
	pre_pre_pre_punish=String.valueOf(Double.parseDouble(curr_punishInterest)-Double.parseDouble(pre_punish)-Double.parseDouble(pre_pre_punish));
	//System.out.println("pre_pre_pre_punish=================================="+pre_pre_pre_punish);
	
	if(Double.parseDouble(pre_pre_pre_rent)>0){
		line_str5="第"+pre_pre_pre_rent_list+"期及之前租金 金额为￥"+formatNumberStr(pre_pre_pre_rent,"#,##0.00")+"元，累计罚息金额为￥"+formatNumberStr(pre_pre_pre_punish,"#,##0.00");
	}
...
<% if(client_address.length()<=30){%>
obj.DrawObject "", "", 0, "578,61", 1, "1,2,3,4", "", "true,1"
...
<%}else{%>
obj.DrawObject "", "", 0, "600,40", 1, "1,2,3,4", "0,0", "true,1"
<%}%>
