...
<td class="tree_title_txt"   valign="middle" align="center">选择项目经理</td>
</tr>
...
{
	wherestr=" and proj_manage_name like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="projManageName.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>项目经理: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr=" select distinct proj_manage_name  from vi_report_contract_fund_fund_charge_plan where proj_manage_name != ''  "+wherestr+" order by proj_manage_name";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('proj_manage_name','proj_manage_name');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("proj_manage_name"))%>" returnlist="<%=getDBStr(rs.getString("proj_manage_name"))%>">
	<%=getDBStr(rs.getString("proj_manage_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('proj_manage_name','proj_manage_name');">  
