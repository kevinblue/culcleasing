...
<td class="tree_title_txt"   valign="middle" align="center">选择收/付款对象</td>
</tr>
...
{
	wherestr=" and pay_obj_name like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="payObjectName.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>收/付款对象: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr=" select distinct pay_obj_name  from vi_report_contract_fund_fund_charge_plan where pay_obj_name != ''  "+wherestr+" order by pay_obj_name";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('pay_obj_name','pay_obj_name');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("pay_obj_name"))%>" returnlist="<%=getDBStr(rs.getString("pay_obj_name"))%>">
	<%=getDBStr(rs.getString("pay_obj_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('pay_obj_name','pay_obj_name');">  
