...
<td class="tree_title_txt"   valign="middle" align="center">选择计划银行账户</td>
</tr>
...
{
	wherestr=" and plan_bank_no like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="dept.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>计划银行账户: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr=" select distinct plan_bank_no  from fund_rent_plan where plan_bank_no != ''  "+wherestr+" order by plan_bank_no";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('plan_bank_no','plan_bank_no');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("plan_bank_no"))%>" returnlist="<%=getDBStr(rs.getString("plan_bank_no"))%>">
	<%=getDBStr(rs.getString("plan_bank_no"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('plan_bank_no','plan_bank_no');">  
