...
{
	wherestr=" and feetype_name like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="fund_feetype.jsp">
...
<%
String sqlstr="select distinct feetype_name from vi_report_ZJCX_fund where 1=1 "+wherestr+" order by feetype_name";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('feetype_name','feetype_name');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("feetype_name"))%>" returnlist="<%=getDBStr(rs.getString("feetype_name"))%>">
	<%=getDBStr(rs.getString("feetype_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('feetype_name','feetype_name');">  
