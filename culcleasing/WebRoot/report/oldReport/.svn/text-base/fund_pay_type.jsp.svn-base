...
{
	wherestr=" and pay_type_name like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="fund_pay_type.jsp">
...
<%
String sqlstr="select distinct pay_type_name from vi_report_ZJCX_fund where 1=1 "+wherestr+" order by pay_type_name";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('pay_type_name','pay_type_name');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("pay_type_name"))%>" returnlist="<%=getDBStr(rs.getString("pay_type_name"))%>">
	<%=getDBStr(rs.getString("pay_type_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('pay_type_name','pay_type_name');">  
