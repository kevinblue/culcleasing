...
<td class="tree_title_txt"   valign="middle" align="center">选择区域</td>
</tr>
...
{
	wherestr=" and plan_bank_name like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="rent_plan_bank.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>区域: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr="select distinct plan_bank_name from vi_report_ZJCX_rent where 1=1 "+wherestr+" order by plan_bank_name";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('plan_bank_name','plan_bank_name');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("plan_bank_name"))%>" returnlist="<%=getDBStr(rs.getString("plan_bank_name"))%>">
	<%=getDBStr(rs.getString("plan_bank_name"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('plan_bank_name','plan_bank_name');">  
