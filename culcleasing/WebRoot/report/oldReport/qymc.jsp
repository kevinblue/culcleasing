...
{
	wherestr=" and qymc like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="qymc.jsp">
...
<%
String sqlstr="select distinct qymc from jb_qyxx where 1=1 "+wherestr+" order by qymc";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('qymc','qymc');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("qymc"))%>" returnlist="<%=getDBStr(rs.getString("qymc"))%>">
	<%=getDBStr(rs.getString("qymc"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('qymc','qymc');">  
