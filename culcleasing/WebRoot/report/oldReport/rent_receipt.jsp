...
{
	wherestr=" and receipt_bank like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="rent_receipt.jsp">
...
<%
String sqlstr="select distinct receipt_bank from vi_report_ZJCX_rent where 1=1 "+wherestr+" order by receipt_bank";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('receipt_bank','receipt_bank');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("receipt_bank"))%>" returnlist="<%=getDBStr(rs.getString("receipt_bank"))%>">
	<%=getDBStr(rs.getString("receipt_bank"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('receipt_bank','receipt_bank');">  
