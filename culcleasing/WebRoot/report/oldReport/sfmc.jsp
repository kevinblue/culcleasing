...
<td class="tree_title_txt"   valign="middle" align="center">选择省份</td>
</tr>
...
{
	wherestr=" and sfmc like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="sfmc.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>省份: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr="select distinct sfmc from jb_ssxx where 1=1 "+wherestr+" order by sfmc";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('sfmc','sfmc');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("sfmc"))%>" returnlist="<%=getDBStr(rs.getString("sfmc"))%>">
	<%=getDBStr(rs.getString("sfmc"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('sfmc','sfmc');">  
