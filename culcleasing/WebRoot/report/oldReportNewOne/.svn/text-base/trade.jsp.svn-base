...
<td class="tree_title_txt"   valign="middle" align="center">选择板块</td>
</tr>
...
{
	wherestr=" and trade like '%"+filtervalue+"%'";
}
%>
<form name="searchbar" action="trade.jsp">
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>业务板块: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
<%
String sqlstr="select distinct trade from base_trade where 1=1 "+wherestr+" order by trade";//name-name 
ResultSet rs = null;
rs=db.executeQuery(sqlstr); 

%>
<select name="datalist" size="15" ondblclick="ListSelect('industry_type_name','industry_type_name');" style="height:100%">
<option returnlist=""></option>
<%
while (rs.next())
{
%>
	<option value="<%=getDBStr(rs.getString("trade"))%>" returnlist="<%=getDBStr(rs.getString("trade"))%>">
	<%=getDBStr(rs.getString("trade"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('industry_type_name','industry_type_name');">  
