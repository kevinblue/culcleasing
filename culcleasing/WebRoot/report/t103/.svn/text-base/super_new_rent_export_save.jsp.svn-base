...
String sql_str =" ";
String col_str = "";
col_str =" qy 区域,dld 代理商,subcompany 分支机构,shortname 担保公司,vt.proj_id 项目编号,khmc 客户名称,cust_id 客户编号,card_id 身份证号,phone 联系方式,";
col_str+=" prod_id 租赁物类型,manufacturer 制造商,method 规则付款,model_id 机型,equip_sn 出厂编号,BodyNo 车架号,";
col_str+=" cast(equip_amount as decimal) 台量,lx_date 申请日期,proj_pz_date 信审批准日期,proj_qy_date 签约日期,check_date 验收日期,";
col_str+=" proj_qzconfirm_date 起租确认日期,cast(lease_term as decimal)租赁期限,end_leasing_date 租赁到期日,equip_delivery_place 交货地点,";
col_str+=" case when vpf.zlwfk_fact_date is null then '0%' else cast(isnull(fk_ratio1,100)+isnull(fk_ratio2,0) as varchar(10))+'%' end 放款比例, ";
col_str+=" case when vpf.zlwfk_fact_date is null then '否' else '是' end 是否放款,vpf.zlwfk_plan_date 计划放款日期,vpf.zlwfk_fact_date 实际放款日期,equip_amt 租赁物购买价款,";
...
col_str+=" lessee_caution_money '6担保费',nominalprice '7留购价款',first_payment '首期付款合计',sale_caution_money '8DB保证金',zdb_bzj 转DB保证金,";
col_str+=" supervision_fee '9管理服务费',first_paytype 首付方式,rent_paytype 租金方式,sfk_plan_date 计划首期付款日,sfk_fact_date 实际首期付款日,";
col_str+=" total_rent 租金总额,total_interest 利息总计,cast((lease_term-un_received_amount) as decimal) 已还租期数,";
col_str+=" (total_rent-left_rent) 已还租金,cast(un_received_amount as decimal)剩余租金期数,left_rent 租金余额,left_corpus 剩余本金,left_interest 剩余利息,overdue_sum 逾期租金,";
col_str+=" overdue_day 逾期天数,total_penalty 违约金合计,paid_penalty 已收违约金,(total_penalty-paid_penalty) 未收违约金,cast(overdue_amount as decimal) 逾期期数,";
col_str+=" (select cast(lj_overdue_amount as decimal) from v_overdue_info where proj_id=vt.proj_id) 累计逾期,";
col_str+=" bank 开户银行, account 开户账号,js_status 结束方式,status_name 状态 ";
