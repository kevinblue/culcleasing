...
<%
	...
	if(rs.next()){
		...
		basic_rate=getStr(rs.getString("base_adjust_rate")); 
	}rs.close();
	// where adjust_id='"+czid+"'
	String wherestr=" where proj_status=20 and method='是' and proj_id not in(select proj_id from adjust_interest_proj where adjust_id='"+czid+"') and proj_id in(select distinct proj_id from fund_rent_plan where plan_status='0' and plan_date>'"+start_date+"')";
	if(!proj_id.equals("")){
	...
	exesqlstr+=" where proj_status=20 and method='是' and proj_id not in(select proj_id from adjust_interest_proj where adjust_id='"+czid+"') and proj_id in(select proj_id from fund_rent_plan where plan_status='0' and plan_date>'"+start_date+"')";
...
<form name="list" action="tx_wait_choice_save.jsp" method="post" target="_blank">
...
&nbsp;&nbsp;
年利率<input name="nll" type="text"  size="8" value="<%=basic_rate %>">
&nbsp;&nbsp;
...
function exportData(){
	if(confirm("是否导出为excel？")){
		...
		list.target="_blank";
		list.submit();
		list.action="tw_wait_choice.jsp";
		list.target="_self";
	}
}
...
function addxm(){ 
	...
	if(j==0){
		...
		list.action="tx_wait_choice_save.jsp";
		list.target="_blank";
		list.submit();
	}
}
function xx_submit(){
	list.action="tx_wait_choice.jsp";
	list.target="";
	list.submit();
}
</script>
