...
function ListSelect(dobj,vobj,vobj2){//05.04.27
	...
	if(i==-1){alert("请先选择！");return false;}
...
<td class="tree_title_txt"   valign="middle" align="center">选择合同号</td>
</tr>
...
<table id="" border="0" cellspacing="0" cellpadding="0" style="float:left " >
      <tr>
        <td nowrap>合同编号: <input type="text" name="fv"  value="<%=filtervalue%>" style="width:90px">
...
{
%>
	<option value="<%=getDBStr(rs.getString("contract_id"))%>" returnlist="<%=getDBStr(rs.getString("project_name"))%>" returnlist2="2222"><!-- 剩余本金先模拟 -->
	项目名称：<%=getDBStr(rs.getString("project_name"))%> -- 合同编号： <%=getDBStr(rs.getString("contract_id"))%>
	</option>
<%} %>
</select>
</div>
...
<tr>
    <td align="center">
    <input  class="btn3_mouseout" type="button" value="确定" onClick="ListSelect('form1.contract_id','form1.project_name','form1.corpus_overage');">  
