...
{
wherestr=wherestr+" and jb_gjxx.id = '"+czid+"' "; 
}
if (request.getParameter("searchKey")!=null)
{
   searchKey=getStr(request.getParameter("searchKey"));
   wherestr=wherestr+" and jb_gjxx.gjmc like '%"+searchKey+"%' "; 
}
sqlstr = "select jb_gjxx.*,base_user.name from dbo.jb_gjxx left join base_user on jb_gjxx.czy=base_user.id"+wherestr+"order by jb_gjxx.id asc"; 
...
<table border="0" cellspacing="0" cellpadding="0" >    
    <tr class="maintab">
		<form name="searchbar" action="gjxx_list.jsp">
		<td nowrap>国家名称：
...
<%if(right.CheckRight("ywjcxx_gjxx_add",dqczy)>0){%>
		<td><a href="#" accesskey="n" onClick="dataHander('add','gjxx_add.jsp',dataNav.itemselect);">
		<img align="absmiddle" src="../../images/sbtn_new.gif" alt="新增(Alt+N)"></a></td><%}%>
<%if(right.CheckRight("ywjcxx_gjxx_mod",dqczy)>0){%>
		<td><a href="#"  accesskey="m" onClick="dataHander('mod','gjxx_mod.jsp?czid=',dataNav.itemselect);">
		<img align="absmiddle" src="../../images/sbtn_mod.gif" alt="修改(Alt+M)" width="19" height="19" align="absmiddle" ></a></td><%}%>
<%if(right.CheckRight("ywjcxx_gjxx_del",dqczy)>0){%>
        <td><a href="#" accesskey="d" onClick="dataHander('del','gjxx_del.jsp?czid=',dataNav.itemselect);">
...
%>
      ...
      <td><input class="rd" type="radio" name="itemselect" value="<%=getDBStr(rs.getString("id"))%>"></td>
        ...
        <td><a href="gjxx.jsp?czid=<%=getDBStr(rs.getString("id"))%>" target="_blank"><%=getDBStr(rs.getString("gjmc"))%></a></td>
        <td><a href="../qyxx/qyxx_list.jsp?czid=<%=getDBStr(rs.getString("id"))%>" target="_blank">查看</a></td>
