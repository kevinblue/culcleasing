...
//try {
    if (request.getParameter("savetype")!=null)
    {
        ...
        if (stype.indexOf("add")>=0)     //新增操作
       {
            ...
            //判断主键是否重复
            sqlstr="select * from jb_zldl where id='"+lbdldm+"'";
			...
			%>
				<script>
                window.history.back();
                alert("所填资料类别大类代码与现有代码重复");
				</script>
			<%
            }
            else
            {
            sqlstr="insert into jb_zldl(id,lbdlmc,gxrq,czy) values ('"+lbdldm+"','"+lbdlmc+"',"+datestr+",'"+czy+"')";
       ...
       if (stype.indexOf("mod")>=0)      //修改操作
       {
		...
		{}
            ...
            if (tempFlag!=0){
                sqlstr="update jb_zldl set id='"+lbdldm+"',lbdlmc='"+lbdlmc +"',gxrq="+datestr+",czy='"+czy+"' where id='"+kid+"'";
       ...
       if (stype.indexOf("del")>=0)         //删除操作
       {
            ...
            sqlstr="delete from jb_zldl where id='"+czid+"'";
            i=db.executeUpdate(sqlstr); 
