...
ResultSet rs;
String sqlstr = "select  dbo.getcustname(info.cust_id) as cust_name,info.contract_id,dbo.fk_getname(leas_mode)as leas_mode,equip.equip_num,dbo.getmodelbyid(equip.device_type)as model_name,equip.equip_sn,expect_start_date,send_date from contract_info as info left join contract_equip as equip on (info.contract_id=equip.contract_id) left join contract_time_info as time on(info.contract_id=time.contract_id) order by expect_start_date desc"; 
%>
