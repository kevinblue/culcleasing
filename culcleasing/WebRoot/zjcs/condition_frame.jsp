...
<form name="form1" method="post" target="_black" action="condition_sc.jsp" onSubmit="return checkInse();">
...
<%
	...
	csa_hours=csa_hours.equals("")?"0":csa_hours;
...
<table  border="0" cellspacing="0" cellpadding="0" width="98%" align="center" class="tab_table_title">
  ...
  <tr>
  	<th>整机价格</th>
    <td><input name="machine_price" value="<%=machine_price %>" type="text"  Require="true" size="15" maxB="20" dataType="Money" onPropertyChange="machine_change();">
  ...
  <tr>
  	<th>预计起租日期</th>
    <td><input name="start_date" value="<%=start_date %>" type="text" dataType="Date" size="10" maxlength="10" maxB="10"  Require="true">
      <img  onClick="openCalendar(start_date);return false" style="cursor:pointer; " src="../../images/tbtn_overtime.gif" width="20" height="19" border="0" align="absmiddle"><span class="biTian">*</span></td>
	<th>租金支付日期</th>
    <td><input name="income_day" value="<%=income_day %>" type="text"   size="10" maxB="20" dataType="Integer" Require="true">
      <span class="biTian">*</span></td>
    <th>首付比例</th>
  ...
  <tr>
    ...
    <td><input name="supervision_fee" value="<%=supervision_fee %>" type="text"   size="15" maxB="20" dataType="Money" onPropertyChange="sfk_change();"></td>
  	<th>名义利率</th>
    <td><input name="year_rate" value="<%=year_rate %>" type="text"   size="15" maxB="20" dataType="Double" Require="ture"> % <span class="biTian">*</span></td>
	<th>逾期罚息率</th>
    <td><select name="pena_rate">
      <script>w(mSetOpt('<%=pena_rate %>',"6.6666|3.8000|8.0000|5.0000"));</script>
    </select>
      %% </td>
  ...
  <tr>
    ...
    <th>净融资额</th>
    <td><input name="lease_money" value="<%=lease_money %>" type="text"   size="15" maxB="20" dataType="Money" readonly  onPropertyChange="fee_change();"></td>
  </tr>
  <tr>
    <th>首付款</th>
    ...
    <td><input name="irr" value="<%=irr %>" type="text"   size="15" maxB="20" dataType="Rate" readonly>
      %</td>
    <th>每期租金</th>
...
function nominalpriceDef(){
	var nominalprice=document.form1.nominalprice.value;
	if(nominalprice==''){
		document.form1.nominalprice.value=10;
	}
}
nominalpriceDef();
function insur_change(){//444
	var insurer=document.form1.insurer.value;//保险公司
	var lease_term=document.form1.lease_term.value;//租赁期限
	var equip_amt=document.form1.equip_amt.value;//租赁物总价
	var model=document.form1.model.value;
	
	if(lease_term!=""&&equip_amt!=""){
		var xmlHttp = GetXmlHttpObject();
		var url = "condition_check.jsp?flag=1&czid="+insurer+"&lease_term="+lease_term+"&equip_amt="+equip_amt+"&model="+model;
...
function fee_change(){
	...
	var lease_money=document.form1.lease_money.value;
	var model=document.form1.model.value;
	
	if(lease_term!=""&&lease_money!=""){
		var xmlHttp = GetXmlHttpObject();
		var url = "condition_check.jsp?flag=2&czid="+insurer+"&lease_term="+lease_term+"&equip_amt="+lease_money+"&model="+model;
	  	//xmlHttp.onreadystatechange = stateChanged;	
	  	xmlHttp.open("POST",url,false);
	  	xmlHttp.send(null);
	  	var vDiv = document.createElement("DIV");
		...
		var returnvalue=vDiv.innerText;
		if(i_temp_scl>5){
			document.form1.supervision_fee.value=returnvalue;
		}else{
			i_temp_scl++;
		}
	}
...
function spr_change(){
	...
	var num = new Number(forcepos(lease_amt*first_payment_ratio/100,2));
	//alert(num.toFixed(0));
	document.form1.first_payment.value=num.toFixed(0);
	document.form1.lease_money.value=forcepos(lease_amt-num.toFixed(0),2);
...
function insurFlag_change(){////////////22222222222222222222
	var insurance_flag=document.form1.insurance_flag.value;
	var csa_financing=document.form1.csa_financing.value;
	var csa_flag=document.form1.csa_flag.value;
	var f1=0;
...
function sfk_change(){//3333333333333333
	var csa_flag=document.form1.csa_flag.value;
	var csa_financing=document.form1.csa_financing.value;
	var insurance_flag=document.form1.insurance_flag.value;
...
function checkInse(){
	...
	var insurer=document.form1.insurer.value;
	var xmlHttp = GetXmlHttpObject();
	var url = "condition_ins_check.jsp?model="+model+"&insurer="+insurer;
	xmlHttp.open("POST",url,false);
	xmlHttp.send(null);
	var vDiv = document.createElement("DIV");
	vDiv.innerHTML = xmlHttp.responseText 
	...
	return Validator.Validate(document.form1,3)
}
</script>