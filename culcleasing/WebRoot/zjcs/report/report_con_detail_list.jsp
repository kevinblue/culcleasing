...
<%
	...
	String wherestr = " where ci.industry_type='电信' and ci.cust_id='"+cust_id+"' ";
	String searchFld_tmp = "";
	if( searchFld.equals("合同编号")){
		searchFld_tmp = "ci.contract_id";
	}
	else if( searchFld.equals("分公司")){
		searchFld_tmp = "vca.cust_name";
	}
	else{
		searchFld_tmp = "";
	}
	...
	String plan_date="";
	//拼装报表的SQL语句
	String sql =  "Select distinct CONVERT(varchar(7), plan_date, 111) as plan_date from fund_rent_plan frp left join contract_info ci on frp.contract_id=ci.contract_id left join vi_cust_all_info vca on ci.cust_id=vca.cust_id "+wherestr;
	...
	rs.close();
	String  sqlstr="select vca.cust_name,ci.contract_id, equip_amt,ci.cust_id from contract_info ci ";
		   	sqlstr+="left join contract_condition cc on ci.contract_id=cc.contract_id left join vi_cust_all_info vca on ";
			sqlstr+="ci.cust_id=vca.cust_id "+wherestr;
...
function isExport() {
	if (confirm("是否确认导出Excel!")) {
		...
		dataNav1.action="report_con_detail_export.jsp";
  		form1.submit();
		dataNav1.action="report_con_detail_list.jsp";
	}
...
<body onLoad="" style="border:1px solid #8DB2E3;overflow:auto">
	 <form name="dataNav1" action="report_con_detail_list.jsp" method="post">		
...
%>
      ...
      <tr>
	    ...
	    for(int m=0;m<l_date_list.size();m++){
	    	...
	    	String sql2="select dbo.bb_getMonthRent('"+contract_id+"','"+l_date_list.get(m)+"',2) as total_rent,dbo.bb_getMonthCorpus('"+contract_id+"','"+l_date_list.get(m)+"',2) as total_corpus,dbo.bb_getMonthInterest('"+contract_id+"','"+l_date_list.get(m)+"',2) as total_interest";
