...
//alert("join");   
   ...
   document.getElementById('save_type').value = "process";
    ...
    dataNav.action = "tx_executeTx_bgz.jsp";
...
<%
	...
	String lxfd = getStr(request.getParameter("lxfd")); 
	...
	sql.append(" select * from vi_tx_showAll_ytx  vts")
	   ...
	   if(!"".equals(lxfd) && lxfd != null){
		  sql.append(" and rate_float_type = '"+lxfd+"' ");//
	   }  
	   sql.append("  and adjust_id='"+txId+"' and settle_method='"+adjust_method+"'");
	   System.out.println("调息界面Sql==>> "+sql.toString());
	   String sqlstr = sql.toString();
%>
<form name="dataNav" id="dataNav" action="tx_showAll_ytx_bgz.jsp" method="post" onSubmit="return goPage()">
<!-- custId == adjust_id  -->
    ...
    <tr class="maintab">
	   ...
	   调息方式
			<select name="adjustStyle">
			  <script type="text/javascript">
			   	w(mSetOpt('<%=adjustStyle %>',"|次日|次期|次年","|次日|次期|次年"));
			  </script>
			</select>
			...
			<select name="lxfd">
			  <script type="text/javascript">
			   	w(mSetOpt('<%=lxfd %>',"|按央行利率浮动比率|按央行利率加点|隐含利率|保持不变",
				"|按央行利率浮动比率|按央行利率加点|隐含利率|保持不变"));
			  </script>
			</select>
			调息状态
