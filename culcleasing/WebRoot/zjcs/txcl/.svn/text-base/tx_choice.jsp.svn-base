...
<form name="list" action="tx_choice_save.jsp" method="post" target="_blank">
...
<%
    sqlstr="select top 300 adjust_interest_proj.*,vi_cust_all_info.cust_name from adjust_interest_proj left join proj_info ";
    ...
    sqlstr+=" where adjust_interest_proj.adjust_id='"+czid+"' and adjust_flag='否' ";// and  datediff(dd,adjust_interest_proj.create_date,getdate())=0
//    sqlstr+=" and adjust_interest_proj.proj_id in(select proj_id from proj_condition where lease_term = 30 and year_rate='8.6')";
//sqlstr+=" and adjust_interest_proj.proj_id in('SFCQDW10043')";

//sqlstr+=" and adjust_interest_proj.proj_id in(";
//sqlstr+=" select proj_id from fund_rent_plan where (plan_date='2010-12-05' or plan_date='2010-12-20')";
//sqlstr+=" and rent_list=2 and proj_id in( ";
//sqlstr+=" select proj_id from proj_info where proj_approvedate<'2010-11-01'";
//sqlstr+="))";
