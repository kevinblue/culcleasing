...
<%
		...
		if(model.equals("del")){//发票删除
			String q_sfp = " select *  from fund_Assets_Invoice where id = '"+key_id+"' ";
			String Fp_num_t = "";//Fp_num
			ResultSet rs_f = db.executeQuery(q_sfp);
			if(rs_f.next()){
				Fp_num_t = rs_f.getString("Fp_num");
			}rs_f.close();
			if(!"".equals(Fp_num_t) || Fp_num_t != null){
				message = "发票编号存在不允许删除,删除";
				flag = -1;
			}else{
				message = "发票删除";
				...
				//删除操作要将资产包表的状态改为‘待提交’ fund_Assets_Package
				if(flag > 0){
					String upda_s = " update fund_Assets_Package set status = '待提交' where Zc_num = '"+Zc_num+"' ";
					flag = db.executeUpdate(upda_s);
				}
			}
	  ...
	  <tr>
	    ...
	    <td colspan="2">
	    	<input type="text" name="Fp_tt" value="<%=UserName%>"  
	    		dataType="Money" size="40" maxlength="50" maxB="50"  Require="true" /> 
