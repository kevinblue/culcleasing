...
<%
	...
	String query_department = " select id from base_department where id = ( ";
	...
	String dept_id = "";
	if(rs_d.next()){
		dept_id = getDBStr( rs_d.getString("id") ) ;
	}
	//ADMN-83C636	资产管理部     ADMN-7ED2ZH  	财务会计部
	if("".equals(dept_id) || dept_id == null){//未有部门
		qx_value = "0";
	}else if ("ADMN-83C636".equals(dept_id)){
		qx_value = "1";
	}else if ("ADMN-7ED2ZH".equals(dept_id)){
		qx_value = "2";
	}
	rs_d.close();
...
<%
	...
	//资产包 表
	String sqlstr = " select *,creator=dbo.GETUSERNAME(caertor) from fund_Assets_Package  order by caertor_date desc"; 
%>
      ...
      <tr>
		...
		<% 
			...
			if("待提交".equals(status) && (sum_rent_jx.equals(sum_rent_fp)) && !qx_value.equals("2")){//资产包状态为‘待提交’并且发票金额相等 并且权限为‘资产管理部’才能进行审核操作
			   status_tem = "提交审核";
		%>
        	<a href="fp_tjsh.jsp?Zc_num=<%=getDBStr(rs.getString("Zc_num"))%>&UserName=<%=getDBStr(rs.getString("UserName"))%>&model=tjshenhe" target="_blank">
        		提交审核
        	</a>
		<%  
			}
			...
			//审核完毕
		%>
        	<a href="fp_tjsh.jsp?Zc_num=<%=getDBStr(rs.getString("Zc_num"))%>&UserName=<%=getDBStr(rs.getString("UserName"))%>&model=shenheOk" target="_blank">
        		审核
        	</a>
		<%  }else{ 
		...
		%>
			   		发票暂不完整请添加
		<%     }
				else if(flag == false && yk_Fp_rate == Double.valueOf("100") && !"待提交".equals(status)){//发票号不完整  
					   status_tem = "发票号不完整";
		%>
					发票号不完整请添加
		<%
		...
		<%
				...
				else if("待提交".equals(status) && qx_value.equals("2")){//财务在资产部未提交审核的前提下进入资产包页则提示请资产部提交审核的显示说明
		%>		
					请资产部提交审核				
		<%
				}
		...
		<td align="center" nowrap>
			<%
				if("待提交".equals(status) && qx_value.equals("2") && yk_Fp_rate >= Double.valueOf("100")){
			 %>
			  	请资产部提交审核
			<% }else{%>
