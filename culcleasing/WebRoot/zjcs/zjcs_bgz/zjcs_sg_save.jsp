...
<%
	...
	if(!stype.equals("del")){
		...
		String query_sumRent = " select sum(corpus_market) as  corpus from fund_rent_plan_proj_temp where  proj_id = '"+proj_id+"' and measure_id = '"+doc_id+"' ";
...
<% 		
	...
	///	} 	
		...
		//now_sumRent = formatNumberDoubleSix(now_sumRent);
		now_sumRent  = Double.valueOf(Math.round(now_sumRent));
		System.out.println("now_corpus--->"+formatNumberDoubleSix(now_corpus));
		System.out.println("corpus--->"+formatNumberDoubleSix(corpus));
		System.out.println("now_sumRent--->"+formatNumberDoubleSix(now_sumRent));
		Double now_lease_money = Double.valueOf(lease_money);
		System.out.println("now_lease_money--->"+formatNumberDoubleSix(now_lease_money));
		if(now_sumRent > now_lease_money){
%>
		<script>
			window.close();
			opener.alert("操作失败：添加的偿还计划本金总和大于租赁本金，不合法!");
			opener.location.reload();
		</script>
<%
			rs.close();
	...
	//###################################################################################################
		...
		if(!stype.equals("del")){
			...
			String now_corpus = "";//偿还计划表中对应主键的所有本金之和 用于和填写的本金相加同 租赁本金比较 
			String query_sumRent = " select sum(corpus_market) as  corpus from fund_rent_plan_temp where  contract_id = '"+contract_id+"' and measure_id = '"+doc_id+"' ";
			//注意:查询本金总和的时候要排除掉当前这期的本金 当前这期本金是以输入的本金为准而不是数据库中原本的这期本金为准
			query_sumRent = query_sumRent + " and rent_list <> '"+rent_list+"' ";
			rs = db.executeQuery(query_sumRent);
	    ...
	    System.out.println("join4偿还计划##################################$%$%$%$%$%$%");
			...
			if(!repeat_flag.equals("1")){
				...
				sqlstr = "insert into fund_rent_plan_temp"+
			       ...
			       "'"+plan_date+"','未回笼',"+rent+","+
			       ""+corpus+","+interest+","+year_rate+" ,corpus_overage_market = '"+corpus_overage_market+"' ";
			    flag = db.executeUpdate(sqlstr);
			    System.out.println("偿还计划添加sqlst==> : r"+sqlstr);
			    message = "偿还计划添加";
			}
