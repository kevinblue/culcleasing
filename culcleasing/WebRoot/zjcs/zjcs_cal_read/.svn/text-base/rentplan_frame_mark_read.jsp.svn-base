...
<%
	...
	if( !"".equals(proj_id) && !"".equals(doc_id) ){
...
//System.out.println("租金测算查询的页面查询SQL……………………"+sqlstr);
	...
	String query_count = "";
	if( !"".equals(proj_id) && !"".equals(doc_id) ){
		query_count = "select sum(rent) as count_rent,sum(corpus_market) as count_corpus,sum(interest_market) as count_interest from fund_rent_plan_proj_temp  where  proj_id='"+proj_id+"' and measure_id='"+doc_id+"'";
	}else{
		query_count = "select sum(rent) as count_rent,sum(corpus_market) as count_corpus,sum(interest_market) as count_interest from fund_rent_plan_proj_temp  where 1=2 and proj_id='"+proj_id+"' and measure_id='"+doc_id+"'";
	}
...
<form name="form1" method="post" target="_black" action="rentplan_sc.jsp" >
