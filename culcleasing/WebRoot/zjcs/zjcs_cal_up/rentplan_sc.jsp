...
<% 
	...
	//新增取值【2010-08-06】
	String accountPrincipal = getZeroStr(getStr(request.getParameter("accountPrincipal")));//会计核算本金
	//新增取值【2010-08-20】
	String rentScale = getZeroStr(getStr(request.getParameter("rentScale")));//圆整到
	
	String liugprice = getZeroStr(getStr(request.getParameter("liugprice")));//留购价 
	...
	rent.setFuture_money(nominalprice);//未来值  留购价  【2010-08-04 修改前是“0”】
	...
	rent.setPlan_date(now_start_date);//起租日  
	...
	//   System.out.println("^^^^^^^^^^^^^^^list_cashRent为"+list_cashRent.get(i));
	...
	//参数一代表：measure_type，参数二代表：上方的list 封装的所有租金列表 
	//参数三 l_plan_date_ 上方数组 封装时间的结果集
	String str_temp = measure_type;
	if(measure_type.trim().equals("0")){
		str_temp = "1";
	}
	...
	//修改交易结构表proj_condition_temp中的计划IRR的值
	String sqlstr = "update proj_condition_temp set plan_irr="+getIrr+",market_irr = "+Markirr+"where proj_id='"+proj_id+"' and measure_id = '"+doc_id+"' ";
	...
	flag = db.executeUpdate(sqlstr);
	sqlstr = "";
	//拼装新的租金测算增加语句，往表fund_rent_plan_proj_temp中将测算后的值插入数据,成功后刷新该页面的父页
	...
	//************************************************************************************************
	//                                           现金流:财务现金流，市场现金流
	//现金流需要的参数有：lease_money-租赁本金 consulting_fee-咨询费 handling_charge-手续费
	//                equip_amt-设备款 other_expenditure-其它支出 caution_money-保证金
	//                return_amt-厂商返佣 other_income-其他收入 first_payment-首付款 
	//				  other_expenditure-其它支出 old_start_date-起租日 
	//				  period_type-1,期初 0,期未的值  l_rent-租金列表List
	//				  l_plan_date-偿还日期列表List proj_id-项目编号 doc_id-文档编号
	...
	System.out.println("===123sqlo==------------------------------------------------123");
	//每次插入之前需删除之前插入的数据
	String del_sql = " delete from fund_proj_plan_temp where proj_id = '"+proj_id+"' and doc_id = '"+doc_id+"'";
	flag = cashFlow.execProjORcontract_xjl(l_plan_date,l_rent,lease_money,
				 ...
				 "fund_proj_plan_temp",before_interest);//
				 
				 
	//市场现金流 
	//得到保证金抵扣租金List rent_list  租金List,caut_money  保证金
	List new_rent = rent.getRentCautNew(l_rent,caution_money);
	String new_del_sql = " delete from fund_proj_plan_mark_temp where proj_id = '"+proj_id+"' and doc_id = '"+doc_id+"'";
	...
	if(!nominalprice.equals("")){//
		...
		if(t_num > 0){//留购价大于0才进行此操作
			...
			query_count_cw = query_count_cw + " and plan_date = ( select max(plan_date) from fund_proj_plan_temp where proj_id = '"+proj_id+"' ) ";
			query_count_cw = query_count_cw + " and id = ( select max(id) from fund_proj_plan_temp where proj_id = '"+proj_id+"' ) ";
			...
			query_count_market = query_count_market + " and plan_date = ( select max(plan_date) from fund_proj_plan_mark_temp where proj_id = '"+proj_id+"' ) ";
			query_count_market = query_count_market + " and id = ( select max(id) from fund_proj_plan_mark_temp where proj_id = '"+proj_id+"' ) ";
