...
<%
	...
	String message = "";
		...
		String measure_type = getStr(request.getParameter("measure_type"));//租金计算方法  等额租金|不规则|平息法 1|0|5
		...
		String management_fee = getDBStr(getStr(request.getParameter("management_fee")));//管理费【注：新增字段 2010-11-11】  
		...
		//环球新增字段
		  ...
		  */
		String into_batch = getDBStr(getStr(request.getParameter("into_batch")));
		String discount_rate = getZeroStr(getStr(request.getParameter("discount_rate")));
		String rate_subsidy = getZeroStr(getStr(request.getParameter("rate_subsidy")));
		String insure_money = getZeroStr(getStr(request.getParameter("insure_money")));
		String consulting_fee_in = getZeroStr(getStr(request.getParameter("consulting_fee_in")));
		String insure_type = getDBStr(getStr(request.getParameter("insure_type")));
		String free_defa_inter_day = getDBStr(getStr(request.getParameter("free_defa_inter_day")));
	...
	if(savaType.equals("add")){
		...
		sql.append("INSERT INTO proj_condition_temp (proj_id ,currency ,equip_amt ,lease_money ,lease_term")
           ...
           .append(" ,insure_type  ")
           .append(" ,insure_money  ")
           .append(" ,consulting_fee_in  ")  
           .append(" ,free_defa_inter_day  ")
           .append(" )")
	...
	if(savaType.equals("mod")){
	   ...
	   sql.append("UPDATE proj_condition_temp ")
          ...
          //culc add new item====
          .append(" ,into_batch = '"+into_batch+"'" )
          .append(" ,amt_return = '"+amt_return+"'" )
          .append(" ,discount_rate = '"+discount_rate+"'" )
          .append(" ,rate_subsidy  = '"+rate_subsidy +"'" )
          .append(" ,insure_type = '"+insure_type+"'" )
          .append(" ,insure_money = '"+insure_money+"'" )
          .append(" ,consulting_fee_in = '"+consulting_fee_in+"'" )
          .append(" ,free_defa_inter_day = '"+free_defa_inter_day+"'" )
	...
	//生成资金计划信息
	//condition.make_proj_fund_fund_charge_plan_temp(proj_id, doc_id, creator, equip_amt, first_payment,
	//		insure_money, return_amt, handling_charge, caution_money, management_fee, consulting_fee_in, consulting_fee,
	//		other_income, other_expenditure, before_interest, discount_rate, rate_subsidy, nominalprice);
	//设备款 equip_amt  首付款 first_payment  保险费 insure_money 厂商返利 return_amt 手续费 handling_charge
	//保证金 caution_money 管理费 management_fee 咨询费收入 consulting_fee_in  咨询费支出 consulting_fee
	//其他收入 other_income 其他支出 other_expenditure 租前息  before_interest 贴现息  discount_rate 利息补贴 rate_subsidy
	//资产余值 nominalprice
	...
	if("0".equals(measure_type)){
		...
		//市场：流入-流出 = 市场现金流净流量
		//流入量：
	    //  management_fee 管理费 caution_money 保证金  handling_charge 手续费 return_amt 厂商返佣   other_income 其他收入   
	    // first_payment 首付款   before_interest 租前息  nominalprice 残值收入 rate_subsidy 利息补贴
	    //流出量： 设备款 equip_amt   咨询费 consulting_fee   其他支出 other_expenditure 保费金额 insure_money 贴现息 discount_rate
	     Double in_money_mark = Double.valueOf(management_fee) + Double.valueOf(caution_money) + Double.valueOf(handling_charge) 
	     	+ Double.valueOf(return_amt) + Double.valueOf(other_income) + Double.valueOf(first_payment) 
	     	+ Double.valueOf(before_interest) + Double.valueOf(nominalprice) + Double.valueOf(rate_subsidy) ;
	     Double out_money_mark = Double.valueOf(equip_amt) + Double.valueOf(consulting_fee) 
	   	    + Double.valueOf(other_expenditure) + Double.valueOf(insure_money) + Double.valueOf(discount_rate) ;
	...
	if(flag != 99){
		...
		if(tem_delay > 0){//延迟期大于0
			...
			l_rent_new = commUtil.getDelayRent(l_rent_new, delay);//财务
		}
...
%>
	        ...
	        <script type="text/javascript">
	        	//window.open('../zjcs_sg/zjcs_sg_list.jsp?proj_id=<%=proj_id%>&doc_id=<%=doc_id%>&temp=proj_zjcs','不规则调整','top=100,left=200,toolbar=no,menubar=no,scrollbars=no,resizable=yes,location=no,status=no');
