...
<div id="TD_tab_0">
	...
	<%
		...
		<td><%if(getDBStr(rsPlan.getString("plan_status")).equals("0")){ %><a target="qs" href="zjzq_fc.jsp?fund_id=<%=getDBStr(rsPlan.getString("id")) %>&volume=<%=getDBStr(rsPlan.getString("rent_list"))%>&contract_id=<%=contract_id %>&doc_id=<%=doc_id %>"><%} %><%=getDBStr(rsPlan.getString("rent_list"))%><%if(getDBStr(rsPlan.getString("plan_status")).equals("0")){ %></a><%} %></td>
		...
		<td><%=formatNumberStr(getDBStr(rsPlan.getString("corpus_overage")),"#,##0.00")%></td>
      </tr>
	<%} %>
    </table>
</form>
