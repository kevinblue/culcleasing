...
<%@page import="com.tenwa.culc.calc.tx.Tx_Process"%>
...
<%
	...
	String str_adjust_style = getStr(request.getParameter("adjust_style"));//获取所有需要调息的调息方式（次日 ，次期）
	...
	Tx_Init tx_init= new Tx_Init();
		flag += tx_init.Tx_Int_add(all_checkbos_value,begin_id_list,adjust_id,czyid);
