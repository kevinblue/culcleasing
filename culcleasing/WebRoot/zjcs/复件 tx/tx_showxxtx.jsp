...
<%
	...
	StringBuffer sql_last = new StringBuffer();//调息后
 	...
 	//2010-12-29 代码新增 查询新旧irr
	String query_rate = " select plan_irr,plan_irr_old,isHG,hgCreateDate,hgCreator from fund_adjust_interest_contract where contract_id = '"+contract_id+"'   and adjust_id = '"+txId+"' ";
	ResultSet rs_r = null;
	...
	System.out.println("财务调息前IRR查询---》"+query_rate);
 	while(rs_r.next()){
 		before_rate = getDBStr(rs_r.getString("plan_irr_old"));
 		after_rate = getDBStr(rs_r.getString("plan_irr"));
...
<td class="tree_title_txt"  height=26 valign="middle" align="center">会计调息前后对比</td>
</tr>
<tr>
...
<input type="hidden" id="sql_where" name="sql_where" value="<%="and mod_reason = '调息'  and status = '前' and measure_id = '"+txId+"'  and  contract_id = '"+contract_id+"'"%>"/>
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="maintab_content_table">  
<% if(!"是".equals(adjust_flag) && !isHG.equals("是")){//调息未回滚
%>	
	...
	<tr scope="row" align="left">
		<td width="50%" class="cwDLRow">调息前irr：<%=before_rate%></td>
		<td>调息后irr：<%=after_rate%></td>
	</tr>
<%}else{//调息已经回滚情况%>	
	<tr scope="row" align="left">
		<td width="50%" class="cwDLRow">调息日期：<%=getDBDateStr(start_date) %>&nbsp;&nbsp;&nbsp;调息开始期项：<%=rent_num %>&nbsp;&nbsp;调息是否完成:<%=adjust_flag %></td>
...
<%}%>
	...
	<tr>
<%
		String query_sum = "  select SUM(rent) as sum_rent,SUM(corpus) as sum_corpus,SUM(interest) as sum_interest from fund_rent_plan_his  where 1=1   ";
		query_sum = query_sum + " and mod_reason = '调息'  and status = '前' and measure_id = '"+txId+"'  and  contract_id = '"+contract_id+"'  ";
...
%>
 ...
 class="maintab_content_table">
   				<tr class="maintab_content_table_title1">
					...
					<th nowrap>承付日期</th>
					<th nowrap>租金</th>
					<th nowrap>租赁投资净额减少额</th><!-- 2010-12-28修改 之前是：本金 -->
					<th nowrap>确认的融资收入</th>
					<th nowrap>租赁投资净额余额</th>
				  </tr>
<%
	while(rs.next()){
%>
				<tr class="cwDLRow" >
					...
					<td align="center" nowrap><%=formatNumberDoubleTwo(getDBStr(rs.getString("corpus")))%></td>
					<td align="center" nowrap><%=formatNumberDoubleTwo(getDBStr(rs.getString("interest")))%></td>
					<td align="center" nowrap><%=formatNumberDoubleTwo(getDBStr(rs.getString("corpus_overage")))%></td>
				  </tr>
<%	
	}
%>	
...
<%
		//2011-01-04日修改 表名从fund_rent_plan_his 修改为 fund_rent_plan
		 query_sum = "  select SUM(rent) as sum_rent,SUM(corpus) as sum_corpus,SUM(interest) as sum_interest from fund_rent_plan_his  where 1=1   ";
		query_sum = query_sum + "    and  contract_id = '"+contract_id+"'  ";
		query_sum = query_sum + " and mod_reason = '调息'  and status = '后' and measure_id = '"+txId+"'  and  contract_id = '"+contract_id+"'  ";
