...
if(oldyear_rate!=null&&!oldyear_rate.equals("")){
	doldRate = Double.parseDouble(oldyear_rate);
}
...
if(savetype.equals("tx")){
	if(volume_id!=null&&!volume_id.equals("")){
	    ...
	    int iCount = 0;
		String sqlstr = "select count(*) as count from fund_rent_plan_temp where contract_id='" + contract_id + "' and measure_id='"+doc_id+"'"; 
		...
		sqlstr = "select * from fund_rent_plan_temp where contract_id='" + contract_id + "' and measure_id='"+doc_id+"' and rent_list>="+volume_id;
		System.out.println(sqlstr);
		...
		rsTemp.close();
		String strAvg = getAdjustRentByCash(alCashTemp,newyear_rate,oldyear_rate,income_number_year);
		...
		sqlstr+=strAvg+",year_rate="+newyear_rate+" where contract_id='" + contract_id + "' and measure_id='"+doc_id+"' and rent_list>="+volume_id;
		System.out.println(sqlstr);
		db.executeUpdate(sqlstr);
		double dall_rent = 0;
		...
		ArrayList alnewRent = getRentPlanInterest(al,dall_rent,dremain_corpus,dnewRate,period_type,int_income_number_year,aladjust);
		if(alnewRent!=null){
		...
		if(iCountTemp>0){
			...
			sql+= ",remain_corpus="+remain_corpus;
			sql+= ",rate_original="+oldyear_rate;
			sql+= ",rate_adjust="+newyear_rate;
...
<script>
			...
			window.location.href = "tx_newrent.jsp?contract_id=<%=contract_id%>&doc_id=<%=doc_id%>&readonly=<%=readonly%>&irr=<%=getIrr%>";
	</script>
