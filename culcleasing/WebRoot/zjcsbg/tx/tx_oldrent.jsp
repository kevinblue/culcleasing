...
<div id="TD_tab_0">
	...
	<%
		...
		<td><%if(getDBStr(rsPlan.getString("plan_status")).equals("未回笼")&&!(readonly!=null&&readonly.equals("1"))){ %><a target="new_plan" href="tx_newrent.jsp?fund_id=<%=getDBStr(rsPlan.getString("id")) %>&volume=<%=getDBStr(rsPlan.getString("rent_list"))%>&contract_id=<%=contract_id %>&rate_date=<%=getDBStr(rsPlan.getString("plan_date")) %>&doc_id=<%=doc_id %>&readonly=<%=readonly %>"><%}%><%=getDBStr(rsPlan.getString("rent_list"))%><%if(getDBStr(rsPlan.getString("plan_status")).equals("未回笼")){ %></a><%} %></td>
