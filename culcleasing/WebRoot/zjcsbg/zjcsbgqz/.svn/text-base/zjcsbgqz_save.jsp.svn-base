...
if(stype.equals("add_deal")||stype.equals("rentplan")){
	...
	sql+=","+(rent!=null&&!rent.equals("")?rent:"0");
	sql+=")";
	System.out.println(sql);
...
if(stype.equals("add")){
	%>
	<script>
			window.location.href = "zjcsbgqz_cdtview.jsp?contract_id=<%=contract_id%>&cs=<%=cs%>&rent=<%=rent%>&doc_id=<%=doc_id%>&readonly=<%=readonly%>&edit_rent=<%=edit_rent%>";
	</script>
	<%
	}else if(stype.equals("add_deal")){
	%>
	<script>
			window.location.href = "zjcsbgqz_rent.jsp?contract_id=<%=contract_id%>&cs=<%=cs%>&rent=<%=rent%>&doc_id=<%=doc_id%>&irr=<%=getIrr%>&allrent=<%=dallrent%>&ex=<%=getEx%>&readonly=<%=readonly%>&edit_rent=<%=edit_rent%>";
	</script>
	<%
	}else if(stype.equals("rentplan")){
	%>
	<script>
			window.location.href = "zjcsbgqz_rent.jsp?contract_id=<%=contract_id%>&doc_id=<%=doc_id%>&irr=<%=getIrr%>&allrent=<%=dallrent%>&ex=<%=getEx%>&readonly=<%=readonly%>&edit_rent=<%=edit_rent%>";
	</script>
	<%}else if(stype.equals("rentdate")){
	%>
	<script>
			window.location.href = "zjcsbgqz_rent.jsp?contract_id=<%=contract_id%>&doc_id=<%=doc_id%>&irr=<%=getIrr%>&readonly=<%=readonly%>&rent=<%=rent%>&edit_rent=<%=edit_rent%>";
	</script>
	<%}
	db.close();
	db1.close();
