...
<%
	...
	rs.close();
	String sumrent = "";
	sqlstr = "select sum(rent) as sumrent from fund_rent_plan_temp where contract_id='"+contract_id+"' and measure_id='"+doc_id+"'";
	System.out.println(sqlstr);
	...
	db.close();
 %>
<form name="form1" method="post" target="con" action="zjcsbght_save.jsp" onSubmit="return Validator.Validate(this,3);">
...
<table  border="0" cellspacing="0" cellpadding="0" width="98%" align="center" class="tab_table_title">
  ...
  <tr>
	...
	<td><input name="caution_money_ratio" type="text" value="<%=caution_money_ratio %>"  dataType="Rate" size="10" maxlength="10" maxB="10"  Require="true" onblur="getMoney(document.forms[0].caution_money_ratio.value,document.forms[0].caution_money,document.forms[0].equip_amt.value);" onPropertychange="fun_margin();">%<span class="biTian">*</span></td>
  	...
  	<td>租赁保证金</td>
    <td><input name="caution_money" type="text" value="<%=caution_money %>"  dataType="Rate" size="20" maxlength="20" maxB="20"  Require="true" onblur="fun_getRadio(document.forms[0].caution_money.value,document.forms[0].caution_money_ratio,document.forms[0].equip_amt.value);fun_marginMoney();"><span class="biTian">*</span></td>
	 </tr>
  <tr>
	<td>承租人保证金比率</td>
  ...
  <tr>
	...
	<td>租金</td>
	<td><input type="hidden" name="allRent" value="<%=sumrent %>" onPropertyChange="fun_contract()"><input type="text" name="rent" size="20" value="<%=rent %>" dataType="Rate" onPropertychange="fun_subRent()"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%if(readonly!=null&&readonly.equals("1")){}else{ %><input type="button" value="预算" onclick="if(Validator.Validate(document.getElementById('form1'),3)){fun_prodRent();}"><%} %></td>
  </tr>	
</table>
...
function fun_period(varValue){
	fun_period_rent(varValue,"zjcsbgqz_check.jsp");
}
function fun_interest(){
	fun_interest_rent("zjcsbgqz_check.jsp");
}

function fun_rentSub(){
	fun_rentSub_rent("zjcsbgqz_save.jsp","rentplan","add_deal");
}
function fun_save(){
		document.forms[0].action="zjcsbgqz_save.jsp";
		document.forms[0].submit();
}
function fun_rentDate(){
	document.forms[0].savetype.value="rentdate";
