	...
	public int init_irr(String irr,String Markirr,String proj_id,String doc_id,String contract_id,String tabelNmae){
		...
		try {
			if("proj_condition_temp".equals(tabelNmae)){//项目交易结构
				sqlstr = "update proj_condition_temp set plan_irr="+tools.formatNumberDoubleScale(irr,8)+"*100,market_irr = "+tools.formatNumberDoubleScale(Markirr,8)+"*100   where proj_id='"+proj_id+"' and measure_id = '"+doc_id+"' ";
			}
			if("contract_condition_temp".equals(tabelNmae)){//合同交易结构
				sqlstr = "update contract_condition_temp set plan_irr="+tools.formatNumberDoubleScale(irr,8)+"*100,market_irr = "+tools.formatNumberDoubleScale(Markirr,8)+"*100  where contract_id='"+contract_id+"' and measure_id = '"+doc_id+"'  ";
			}
			flag = db.executeUpdate(sqlstr);	
