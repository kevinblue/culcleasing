 ...
 * 
 */
public class RentDelayGraceCalc {
	...
	private String delay="0";//延期期数
	private String grace ="0"; //宽限期数
	

	private HashMap hm = new HashMap();

	public RentDelayGraceCalc() {

	}

	public RentDelayGraceCalc(String year_rate, String income_number,
			String lease_money, String future_money, String period_type,
	 ...
	 * 
	 */
	public List eqRentList(String p_year_rate,int grace) {
		...
		//先添加宽限期的租金
		//宽限期租金
		String g_rent = String.valueOf(Double.parseDouble(rate)*Double.parseDouble(lease_money_));
		 g_rent = Tools.formatNumberDoubleScale(g_rent, Integer
				.parseInt(this.rentScale));
		for (int j =0 ;j<grace;j++) {
			l_rent.add(g_rent);
		}
...
//		得到宽限期
		int grace = Integer.parseInt(this.grace);
		for(int i = 0; i < grace; i++) {
			interests.add(rentList.get(i));
		}

		//正常的租金计划利息
		for (int i = grace; i < rentList.size(); i++) {// 循环租金list
		//for (int i = 0; i < rentList.size(); i++) {// 循环租金list

			if ("1".equals(qzOrqm) && i == grace) {// 第一期且是期初时
	 ...
	 * @return
	 */
	public List getPlanDateList(List rentList, String perType,int delay) {
	 ...
	 * @return
	 */
	public List getDelayDateList(String perType,int delay) {
	...
	public String getIrr(String type, List rent_list, List l_plan_date)
		...
		//延期的租金值添加,租金为0
		for (int i = 0; i <Integer.parseInt(this.getDelay()); i++) {
			rent_list_1.add("0");
		}
		//添加正常的租金
		...
		//添加延期的时间值
		l_plan_date_1 = this.getDelayDateList(this.period_type,Integer.parseInt(this.getDelay()));
		...
		List l_plan_date_cw = new ArrayList();
		List l_rent_cw = new ArrayList();

		// 延期时期
		l_plan_date_cw = this.getDelayDateList(this.period_type, Integer
				.parseInt(this.getDelay()));

		// 添加正常的租金时间
		for (int i = 0; i < l_plan_date.size(); i++) {
			l_plan_date_cw.add(l_plan_date.get(i));
		}
		...
		if ("1".equals(type)) {// 每月计算
			...
			// 租金
			irr.getPreMonthCash(l_rent_cw, l_plan_date_cw);
			// 后期的租金流
			...
			// 租金
			irr.getRentCashFlow(l_plan_date_cw, l_rent_cw,grace);
			// 后期的租金流
			irr.getRentAfterCashFlow(afterDate, afterCash);
			// 时间，资金流的处理类
			irr.getUniqueByDate();
	...
	public String getMarkIrr(String type, List rent_list, List l_plan_date)
		...
		if ("1".equals(type)) {// 每月计算
			...
			// 租金
			irr.getRentCashFlow(l_plan_date, rent_list,grace);
			// 后期的租金流
			irr.getRentAfterCashFlow(afterDate, afterCash);
			// 时间，资金流的处理类
			irr.getUniqueByDate();
	...
	public HashMap getPlanByEqRent(String p_year_rate, List rent_c_list) {
		...
		List l_corpus_overage = this.getCorpusOvergeList(this.cle_cau_Money,
				l_corpus);
		List l_plan_dt = this.getPlanDateList(l_rent, this.period_type,Integer.parseInt(this.delay));
	...
	public HashMap getPlanByEqCorpus() throws Exception {
		...
		List l_plan_dt = this.getPlanDateList(l_rent, this.period_type,Integer.parseInt(this.delay));// 计划时间
		...
		irr.getPreCashFlow(preDate, preCash);
		// 租金
		irr.getRentCashFlow(l_plan_dt, l_rent,grace);
		// 后期的租金流
		irr.getRentAfterCashFlow(afterDate, afterCash);
		// 时间，资金流的处理类
		irr.getUniqueByDate();
		...
		irr.getPreCashFlow(preDate_1, preCash_1);
		// 租金
		irr.getRentCashFlow(l_plan_dt_1, l_rent_1,grace);
		// 后期的租金流
		irr.getRentAfterCashFlow(afterDate, afterCash);
		// 时间，资金流的处理类
		irr.getUniqueByDate();
	...
	public static void main(String[] args) throws Exception {
		...
		rent_list.add("67580");
		rent_list.add("67580");
		rent.getInterestList(rent_list,"200000","0.012376","0");
//		rent.setYear_rate("6.89");
...
//		rent.setCle_cau_Money("10000");
//		rent.setCauti_Money("1000");
//		rent.delay="3";
//		rent.setGrace("3");
//		List l_rent_1 = new ArrayList();
...
//		l_plane.add("2009-12-18");
//		rent.setPreCash(l_rent_1);
//		rent.setPreDate(l_plane);
//
//		// 得到租金list,注意不规则时的租金list
//		List rent_list = rent.eqRentList(rent.year_rate,3);// 规则时
//		List l_plan_date_ = rent.getPlanDateList(rent_list, rent.period_type,3);// 计划时间
//
