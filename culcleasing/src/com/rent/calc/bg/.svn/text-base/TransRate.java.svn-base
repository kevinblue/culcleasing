...
public class TransRate {
	...
	public HashMap getProj_ConditionInfoByProj_id(String proj_id, String term)
		...
		if (rs.next()) {
			...
			hmp.put("net_lease_money", rs.getString("net_lease_money"));
	...
	public void processInterest(String adjustId, List projs) throws Exception {
		...
		for (int i = 0; i < projs.size(); i++) {
			...
			if (Integer.parseInt(rent_list_map.get("nextTerm").toString()) <= Integer
				...
				String nominalprice = condtion_info.get("nominalprice").toString();
				String net_lease_money = condtion_info.get("net_lease_money").toString();
				...
				if (!is_fix_value) {// 不是固定添加租金时,等额租金时
					...
					mp.put("nominalprice", nominalprice);
					mp.put("caution_money", caution_money);
					mp.put("net_lease_money", net_lease_money);
					// 更新租金计划
	...
	public void calRentByNew(HashMap mp) throws Exception {
		...
		rent.setFuture_money("-"+mp.get("nominalprice").toString());
		rent.setStart_date(mp.get("start_date").toString());
		//rent.setCle_cau_Money(String.valueOf(Double.parseDouble(mp.get("net_lease_money").toString())+Double.parseDouble(mp.get("caution_money").toString())));
		rent.setCauti_Money(mp.get("caution_money").toString());
		...
		rent.setCle_cau_Money(Tools.formatNumberDoubleTwo(mp.get("rem_corpus")
				.toString()));
		...
		System.out.println("Marketirr:" + rent.getMarket_irr());
