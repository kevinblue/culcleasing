...
public class Tx_DataCtrolUtil {
	...
	public Map getConditionInfo(String contract_id,String begin_id,String txId) throws SQLException{
		...
		if(rs.next()){
			...
			RentPlan_Map.put("rent_list_start", rs.getString("rent_list_start"));//开始期次
			RentPlan_Map.put("ratio_param", rs.getString("ratio_param"));// 公差/比值
			RentPlan_Map.put("rate_float_type", rs.getString("rate_float_type"));// 利率浮动类型
		}
		erpDataSource.close();
		return RentPlan_Map;
	}
...
System.out.println("第六步:把调息后的数据从his表拷到正式表======"+sql);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map getRentInfoMap(String contract_id,String begin_id,String rent_list_start) throws SQLException{
		...
		String sql = "select rent,corpus,corpus_overage from fund_rent_plan where contract_id='"+contract_id+"' " +
				"and begin_id= '"+begin_id+"' and rent_list>'"+rent_list_start+"'  order by rent_list";
	...
	public void updateRentAfterCancle(String begin_id, String txId) throws SQLException {
		/*String sql = "update fund_rent_plan set interest=(select interest from fund_rent_plan_his " +
				"where begin_id='"+begin_id+"' and tx_id='"+txId+"' and status='前'), "+
				" rent=(select rent from fund_rent_plan_his " +
	...
	String sql = 
		...
		" from fund_rent_plan_his tt where "+
		 " tt.begin_id='"+begin_id+"' and tt.tx_id='"+txId+"' and tt.status='前' and tt.rent_list =(select rent_list_start from fund_adjust_interest_contract where begin_id='"+begin_id+"' and adjust_id='"+txId+"')"+
