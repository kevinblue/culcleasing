 ...
 * 
 * @author toybaby
 * Date:Aug 8, 20114:05:12 PM       Email: toybaby@mail2.tenwa.com.cn
 */
public class Tx_Process {
	...
	/*public static void main(String[] args) throws Exception {
		try {
			Tx_Process Tx_Process = new Tx_Process();
			Tx_Process.tx_ProcessInfo("14CULC022538L64", "14CULC022538L6422", "RentCalcType9", "129", "admin", "次年");
			//int x = Tx_Process.tx_CancleInfo("13CULC011667L55","13CULC011667L5579","129","次年");
	...
	public  int tx_ProcessInfo(String contract_str,String begin_str,String settle_str,
					String txId,String czyid,String adjust_style_str) throws Exception{
		int flag = 0;
		...
		for(int i = 0; i<contract_list.length;i++){
			...
			if("次期".equals(adjust_style[i])){
					...
					if("2".equals(rate_float_type)){//隐含利率调息
						...
						rentInfoBox = choiceSettleMethod(contract_list[i],begin_id[i],settle_method_list[i],condition_Map);
						//第四步:把调息后的数据拷到His表中
			//			System.out.println("开始期数========"+rent_list_start);
		...
		}else if("次日".equals(adjust_style[i])){
			 //	1.当期的单独处理，次期的整个过程都要单独改写，留意当期的特殊情况
			 // 2.次日中的特例：只有最后一期时，次日只能单独处理，不再调用次期的方法计算租金
			 // 3.第一期的次日调息也需单独考虑
			//################
		...
		//			System.out.println("执行成功"+flag+"条");
			...
			int rent_list_start = Integer.parseInt(condition_Map.get("rent_list_start"));//开始期数
			String rate_float_type = condition_Map.get("rate_float_type");//利率浮动类型
			if(!"2".equals(rate_float_type)){ //非隐含利率调息
					//非最后一期或是
					if(max_list!=rent_list_start){
					...
					//次期部分结束
					}else if(max_list==rent_list_start){
						//第七步:把调息记录表中的调息状态更新为是
						tx_DataCtrolUtil.updateFundAdjust(contract_list[i],begin_id[i],txId);
						//第八步:调交易结构条件中的年利率更新成最新的年利率
						tx_DataCtrolUtil.updateConditionForProcess(contract_list[i],begin_id[i], txId);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Tx_RentInfoBox choiceSettleMethod(String contract_id,String begin_id,String settle_method,Map condition_Map) throws SQLException{
