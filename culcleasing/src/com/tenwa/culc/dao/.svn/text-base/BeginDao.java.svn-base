...
public class BeginDao {
	 ...
	 * @throws SQLException
	 */
	public BeginInfoBean loadBeginInfoBeanByKey(String begin_id, String doc_id)
			throws SQLException {
		...
		// 2.拼接sql，执行插入
		String sqlStr = SqlGenerateUtil.generateSelectBeginInfoTemp(begin_id,
				doc_id);
		...
		if (rs.next()) {
			...
			beginInfoBean.setRate_float_amt(String.valueOf(Double.parseDouble(rs.getString("rate_float_amt"))));
			...
			beginInfoBean.setBegin_order_index(rs.getString("begin_order_index"));
			beginInfoBean.setInto_batch(rs.getString("into_batch"));
			beginInfoBean.setInvoice_type(rs.getString("invoice_type"));
			beginInfoBean.setFactoring(rs.getString("factoring"));
