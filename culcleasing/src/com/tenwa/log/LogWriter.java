...
public final class LogWriter {
	...
	public static void logDebug(HttpServletRequest request, String info) {
		// debug信息
		if ("open".equals(ConfigReader.getResultByKey("outlog"))) {
			System.out.println(LogWriter.getLogInfo(request) + "\t Debug："
					+ info);
		}
	}
	...
	public static void logonLog(HttpServletRequest request) {
		...
		// 写登陆信息
		pw.println(LogWriter.getLogInfo(request) + "\t登陆了");
		// 关闭资源
		pw.close();
	}
	...
	public static void logonOutLog(HttpServletRequest request) {
		...
		// 写退出信息
		pw.println(LogWriter.getLogInfo(request) + "\t退出了");
		// 关闭资源
		pw.close();
	}
	...
	public static void operationLog(HttpServletRequest request, String moduel,
		...
		// 写操作日志信息
		String baseInfo = LogWriter.getLogInfo(request);
	 ...
	 * @param info
	 */
	public static void operationFILogNc(String info,String msg, String xmlStr,String faleName) {
		String date = new SimpleDateFormat("yyyy年MM月dd日").format(new Date());

		
		File file = new File((String) ConfigReader
				.getResultByKey("logFilePath")
				+ "/" + date + faleName+".txt");// 创建日志文件
		...
		pw.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
				.format(new Date())
				+ "时间，同步数据Info：" + info+"模块：" + msg + " =========传输的XML字符串========\n"
				+ xmlStr);
		// 关闭资源
		pw.close();
	}
