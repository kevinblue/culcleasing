...
public class CommonUtil {
	...
	public long getDateDiff(Date bdate, Date edate) // 返回两时间间隔天数 bdate--开始时间字符串
	// edate--结束时间字符串
	{
		try {
	 ...
	 * @return
	 */
	public long getDateDiff(String strbdate, String stredate) // 返回两时间间隔天数
	{
		Date bdate = null;
		Date edate = null;
		try {
	...
	public String formatBooleanStr(String str, int mode) // boolean字符串中文处理,true/flase或1/0转换为是/否,mode=0
	// 0转为是,mode=1 1转为是
	{
		try {
			String temp_bs = str;
	...
	public String getPMT(String Rate, String Nper, String Pv, String Fv,
		...
		int Nper_i = Integer.valueOf(Nper).intValue();// 租赁期限

		try {
			// multiply -- 1.multiply(2) = 1*2
			// add -- 1.add(1.5) = 1+1.5
			// pow -- 2.pow(3) = 2*2*2
			// (-1 * 年利率) * ((融资金额 * (1+年利率)*租赁期限次方)+0) / ( ((1+(年利率*1)) *
			// (租赁期限次方 - 1) ) ) 四舍五入
	...
	public ArrayList getRentPlanInterest(ArrayList al, double dallRent,
		...
		// 生成等额租金租金计划;al租金回笼阶段；dallRent租金总额，由回笼阶段获取；dlease_money本金总额，由交易结构获取；income_number，年还租次数；charge_first_date首期租金回笼日；rent_first_date二期租金回笼日；ivolume
		// 开始期数，新增为1，如果不是从1开始应传入起始期数,interval还租间隔时间=12/income_number
	...
	public String getSecondDateByFirstDate(String firstDate,
		...
		lease_interval = "1";
		String first_day = firstDate.substring(8, 10);// 提取出验收日期的天数
		String secondDate = "";
		if (Integer.parseInt(first_day) >= 1
				&& Integer.parseInt(first_day) <= 5) { // 1-5之间 下个月5日收第二期租金
			...
			secondDate = secondDate.substring(0, 7) + "-05";
				...
				&& Integer.parseInt(first_day) <= 20) {// 5--20 下个月20日收租金
			secondDate = getDateAdd(firstDate,
					Integer.parseInt(lease_interval), "mm");
