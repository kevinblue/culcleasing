 ...
 */
public class GlobalConfirmInvokeTask extends TimerTask {
	...
	public void run() {
		...
		InputStream is = null;
		String startTaskGlobalConfirm = "" ;
		try {
			is = new FileInputStream(realpath);
			properties.load(is);
			startTaskGlobalConfirm = properties.getProperty("startTaskGlobalConfirm");
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			if (is != null) {
		...
		System.out.println("&&&&&&startTaskGlobalConfirm&&&& :  "+startTaskGlobalConfirm);
		// 1.判断是否有效时间内 - 2点
		if (monTh ==18 && hourTh==2 && "on".equals(startTaskGlobalConfirm)) {
			System.out.println("代码执行到===");
			// 使用多线程调用WebService
			GlobalConfirmInvokeThread invokeThread = new GlobalConfirmInvokeThread(
					"======[[GlobalInterest Task Thread]]=========");
			invokeThread.setPriority(5);// 设置线程优先级别
			invokeThread.start();
			System.out.println("代码执行到aaaaaaaa");
		} else {
			LogWriter.logDebug("[计提利息 数据同步任务]非有效时间内执行代理nc");
		}
	}
}
