...
import com.webService.service.GlobalBjjsYYXml;
...
String sqlStr = SqlGenerateFIUtil.generateFIDataSyncDBLog(oper_id,
		amount, "DATA_SYNC_GLOBAL_BJJS_YY_NC", "NC利息计算表财务接口数据同步",operRmark);
...
erpDataSource.close();
	...
	private void writeDataSyncDBInfo(
		...
		// 1.遍历所有List数据
			...
			sqlStr = SqlGenerateFIUtil.generateFIDataSyncDBInfo(
					...
					"vi_INTERFACE_fina_global_bjjs_YY_nc");
	...
	public List<GlobalBjjsBean> readGlobalBjjsData(String syncType,String sqlIds)
		...
		ERPDataSource erpDataSource = new ERPDataSource();
		// 2.查询Client1数据库
		String sqlStr = SqlGenerateFIUtil.generateSelectERPGlobalBjjsYYData(sqlIds);
		LogWriter.logSqlStr("读取ERP批量本金计税(营业税)表信息", sqlStr);
	...
	public Map<String,Integer> insert2OracleData(List<GlobalBjjsBean> globalBjjsList,
		...
		if (globalBjjsList.size() < 1) {
			...
			i=0;
			GlobalBjjsYYXml globalBjjsYYXml = new GlobalBjjsYYXml();
			...
			for (GlobalBjjsBean globalBjjsBean : globalBjjsList) {
				...
				try {
					//组装xml字符串
					xmlMassage = globalBjjsYYXml.getXmlStr(globalBjjsBean);
					//请求nc服务端
					...
					if("0".equals(ifsuccess)){
						...
						try {
							...
							LogWriter.operationFILogNc("本次执行[" + syncType + "]数据同步，数据传输成功",
									"插入数据到nc财务接口数据库-利本金计税表（应收）营业税接口", "XML如下:"+xmlMassage,"本金计税表（应收）营业税NC数据同步日志");
						} catch (Exception e2) {
							e2.printStackTrace();
						}
					}else{
						if(key){
							...
							try {
								...
								LogWriter.operationFILogNc("本次执行[" + syncType + "]数据同步，数据传输失败",
										"插入数据到nc财务接口数据库-本金计税表（应收）营业税接口", "XML如下:"+xmlMassage,"本金计税表（应收）营业税NC数据同步日志");
							} catch (Exception e2) {
								e2.printStackTrace();
							}
						}
